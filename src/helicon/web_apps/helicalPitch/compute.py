...
def update_particle_locations(params, apix_micrograph):
    ret = params.copy()
    ret["rlnCoordinateX"] = (
        params.loc[:, "rlnCoordinateX"].astype(float) * apix_micrograph
    )
    ret["rlnCoordinateY"] = (
        params.loc[:, "rlnCoordinateY"].astype(float) * apix_micrograph
    )
...
def get_class2d_params_from_file(params_file, cryosparc_pass_through_file=None):
    ...
    required_attrs = np.unique(
        "rlnImageName rlnHelicalTubeID rlnHelicalTrackLengthAngst rlnCoordinateX rlnCoordinateY rlnClassNumber rlnAnglePsi".split()
    )
