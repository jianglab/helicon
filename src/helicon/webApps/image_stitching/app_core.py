...
with ui.div(
    ...
    def generate_image_transformation_uis():
        ...
        for i, idx in enumerate(selected_images_labels()):
            ...
            ui.insert_ui(
                shiny.ui.row(transformation_ui_group(f"t_ui_group_{idx}")),
            ...
            @reactive.effect
            @reactive.event(
                input[f"t_ui_group_{idx}_pre_rotation"],
                input[f"t_ui_group_{idx}_shift_y"],
                ignore_init=True,
            )
            def transform_selected_images():
                ...
                if (
                    input[f"t_ui_group_{idx}_pre_rotation"]() != 0
                    or input[f"t_ui_group_{idx}_shift_y"]() != 0
                ):
                    rotated[curr_img_idx] = helicon.transform_image(
                        image=rotated[curr_img_idx].copy(),
                        rotation=input[f"t_ui_group_{idx}_pre_rotation"](),
                        post_translation=(input[f"t_ui_group_{idx}_shift_y"](), 0),
                    )
