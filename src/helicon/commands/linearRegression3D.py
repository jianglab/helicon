...
try:
  ...
  helicon.color_print(f"WARNING: failed to load numba. The program will run correctly but will be much slower. Run 'pip install numba' to install numba and speed up the program")
  def jit(*args, **kwargs):
    return lambda f: f
...
def main(args):
  ...
  if input_file.suffix in [".star", ".mrcs", ".mrc"]:
    ...
    if input_file.suffix in [".star"]:
      ...
      if args.i:
        ...
        if len(bad_i):
          logger.error(f"ERROR: image {' '.join(map(str, bad_i))} out of the valid range [1, {nz}]")
          sys.exit(-1)
      ...
      for i in indices:
        ...
        if imageFile.suffix in [".mrc"]:
          imageFile, imageFileOrig = pathlib.Path(imageFile.with_suffix(".mrcs").name), imageFile
          if imageFile.exists(): imageFile.symlink_to(imageFileOrig)
        import mrcfile
      ...
      if input_file.suffix in [".mrc"]:
        input_file, input_fileOrig = pathlib.Path(input_file.with_suffix(".mrcs").name), input_file
        if not input_file.exists(): input_file.symlink_to(input_fileOrig)
      import mrcfile
    ...
    image = simulate_helical_projection(n=n, twist=twist, rise=rise, csym=csym, helical_diameter=helical_diameter, 
                    ball_radius=ball_radius, polymer=polymer, planarity=planarity, ny=ny, nx=nx, apix=apix, tilt=tilt, rot=rot, psi=psi, dy=dy)
    if noise>0:
  ...
  return_3d = not (len(tr_pairs)*len(args.tilts)*len(args.psis)*len(args.dys)>1)
  ...
  for ti, t in  enumerate(itrtpy_list):
    ...
    if abs(twist)<0.01:
      ...
      continue
    if abs(rise)<0.01:
      logger.warning(f"WARNING: (twist={round(twist, 3)}, rise={round(rise, 3)}) will be ignored due to very small rise value (rise={round(rise, 3)}Å)")
      continue
    if abs(rise)>=(args.tube_length/2 if args.tube_length>0 else nx*apix/2):
      logger.warning(f"WARNING: (twist={round(twist, 3)}, rise={round(rise, 3)}) will be ignored due to very large rise value (rise={round(rise, 3)}Å)")
      continue
    if abs(tilt)>45:
  ...
  if args.reconstruct_length_pitch>0:
    np.random.shuffle(tasks)  # spread jobs using a large amount of memory (e.g. long pitches)
  ...
  results = list(
      tqdm(
          ...
          unit = "job",
          total=len(tasks),
          disable=len(tasks)<2 or (args.cpu>1 and args.verbose<2) or (args.cpu==1 and args.verbose!=2)
      )
  )

  end_time = datetime.datetime.now()
  logger.info(f"{len(tasks)} reconstruction jobs finished at {end_time}. Runtime = {end_time-start_time}")

  results_none = [res for res in results if res is None]
  if len(results_none):
    logger.info(f"{len(results_none)}/{len(results)} results are None and thus discarded")
    results = [res for res in results if res is not None]

  results.sort(key=lambda x: x[2][1:])

  if return_3d:
    mapFiles = saveMaps(results, target_size=(nx, ny, ny), target_apix=apix, mapFilePrefix=args.output_prefix, verbose=args.verbose)
    if len(mapFiles)>1:
      s = "\n" + "\n".join(mapFiles)
  ...
  if args.save_projections:
    lstFiles = writeLstFile(results, top_k=args.top_k, apix2d_orig=apix, lstFilePrefix=args.output_prefix)
    if len(lstFiles)>1:
      s = "\n" + "\n".join(lstFiles) + "\n"
      logger.info(f"Reconstructed map projections saved to {s}Use 'e2display.py' to display the images")
    else:
      logger.info(f"Reconstructed map projections saved to {' '.join(lstFiles)}. Use 'e2display.py {' '.join(lstFiles)}' to display the images")

  pdfFiles = writePdf(results, pdfFilePrefix=args.output_prefix, top_k=args.top_k, use_pitch=len(args.pitches)>0, image_info=info, cmap=args.color_map)
  if len(pdfFiles)>1:
    s = "\n" + "\n".join(pdfFiles)
    logger.info(f"Reconstructed map projections/scores saved to {s}")
  else:
    logger.info(f"Reconstructed map projections/scores saved to {' '.join(pdfFiles)}")
    
  if len(images)>1:
    pdfFile = plotAllScores(results, pdfFilePrefix=args.output_prefix, use_pitch=len(args.pitches)>0)
    if pdfFile:
...
def process_one_task(ti, ntasks, data, imageFile, imageIndex, twist, rise, rise_range, csym, tilt, tilt_range, psi, dy, apix2d_orig, denoise, low_pass, transpose, horizontalize, target_apix3d, target_apix2d, thresh_fraction, positive_constraint, tube_length, tube_diameter, tube_diameter_inner, reconstruct_length, sym_oversample, interpolation, fsc_test, return_3d, verbose, algorithm):
  ...
  if target_apix3d < 0:
    vol = reconstruct_length * (reconstruct_diameter**2-reconstruct_diameter_inner**2)/4 * np.pi
  ...
  reconstruct_diameter_3d_inner_pixel = int(round(tube_diameter_inner/target_apix3d))
  ...
  reconstruct_length_2d = tube_length if 0<tube_length<nx*target_apix2d else nx*target_apix2d  # direction of helical axis
  ...
  if reconstruct_length>0:
    reconstruct_length_3d_pixel = max(int(np.ceil(rise/target_apix3d)), int(np.ceil(reconstruct_length/target_apix3d)))
  ...
  if sym_oversample<=0:
    ...
    ratio = 2**20/n_voxels
    if ratio<10: sym_oversample = max(1, int(round(ratio)))
  ...
  if rec3d_y_proj_max>0: rec3d_y_proj *= rec3d_x_proj.max()/rec3d_y_proj_max
...
def lsq_reconstruct(projection_image, scale2d_to_3d, twist_degree, rise_pixel, csym=1, tilt_degree=0, psi_degree=0, dy_pixel=0, thresh_fraction=-1, positive_constraint=-1, reconstruct_diameter_3d_inner_pixel=0, reconstruct_diameter_2d_pixel=-1, reconstruct_diameter_3d_pixel=-1, reconstruct_length_2d_pixel=-1, reconstruct_length_3d_pixel=-1, sym_oversample=1, interpolation="nn", fsc_test=0, verbose=0, algorithm=dict(model="lsq")):
  ...
  def split_A_b(A, b, b_id, mode):
    ...
    A_set_2 = A[~is_set_1]
    b_set_2 = b[~is_set_1]
    assert len(b_set_1)+len(b_set_2)==len(b), f"ERROR: {len(b_set_1)=:,}\t{len(b_set_2)=:,}\t{len(b)=:,}"
    return (A_set_1, b_set_1), (A_set_2, b_set_2)

  #@memory.cache(ignore=["verbose"])
  def solve_equations(A_data, b_data, A_hsym, b_hsym, positive=False, algorithm="elasticnet", train_fraction=1.0, verbose=0):
    ...
    if algorithm["model"] == "lsq":  # ordinary linear least square without regularization
      ...
      res = lsq_linear(A, b, bounds=(lb, ub), tol=tol, max_iter=max_iter, lsmr_maxiter=1000, lsmr_tol='auto', verbose=verbose)
      return res.x.astype(np.float32), None
    
    if algorithm["model"] == "lreg":  # ordinary linear least square without regularization
      ...
      model = Lasso(alpha=algorithm.get("alpha", 1e-4), fit_intercept=True, positive=positive, selection='random', tol=tol, max_iter=max_iter)
    elif algorithm["model"] == "elasticnet":  # linear least square + L1/L2 regularization 
      # https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.ElasticNet.html
      from sklearn.linear_model import ElasticNet
      model = ElasticNet(alpha=algorithm.get("alpha", 1e-4), l1_ratio=algorithm.get("l1_ratio", 0.5), fit_intercept=True, positive=positive, selection='random', tol=tol, max_iter=max_iter)
    elif algorithm["model"] == "ridge":  # linear least square + L2 regularization 
      # https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.Ridge.html
      from sklearn.linear_model import Ridge
      model = Ridge(alpha=algorithm.get("alpha", 1), fit_intercept=True, positive=positive, tol=tol, max_iter=max_iter)
      ...
      A_train = A_train.toarray()
      model = ARDRegression(alpha_1=algorithm.get("alpha", 1e-6), alpha_2=algorithm.get("alpha", 1e-6), fit_intercept=True, tol=tol, max_iter=max_iter)
  ...
  if fsc_test>=1:
    (A_data_set_1, b_data_set_1), (A_data_set_2, b_data_set_2) = split_A_b(A_data, b_data, b_data_pid, mode = fsc_test)

    Ab_pairs = [(A_data_set_1, A_hsym, b_data_set_1, b_hsym,'Half-1'), (A_data_set_2, A_hsym, b_data_set_2, b_hsym, 'Half-2')]
    ...
    Abx_data_triplets += [(A_data_set_1, b_data_set_1, xs[1]), (A_data_set_2, b_data_set_2, xs[2])]
  ...
  shape = (reconstruct_length_3d_pixel, reconstruct_diameter_3d_pixel, reconstruct_diameter_3d_pixel)
...
@memory.cache(ignore=['verbose'])
def build_A_helical_sym_matrix(nz:int, ny:int, nx:int, twist_degree:float, rise_pixel:float, csym:int, rmin:float, rmax:float, min_sym_pairs:int, interpolation:str, verbose:int=0):
  ...
  if interpolation in ["linear", "linear01", "linear11"]:
    ...
    def loop_kji(Zi, Yi, Xi, Zj, Yj, Xj, mask, mask_nonzero_indices_zyx_tuple, mask_nonzero_indices_matrix, pair_ids):
      mask_indices_Z, mask_indices_Y, mask_indices_X = mask_nonzero_indices_zyx_tuple
      mz, my, mx = mask.shape
      ...
      for mi in range(n_indices):
        ...
        yj = int(Yj[k,j,i])
        xj = int(Xj[k,j,i])
        if zi<0 or zi>mz-1: continue
        if zj<0 or zj>mz-1: continue
        if yi<0 or yi>my-1: continue
        if yj<0 or yj>my-1: continue
        if xi<0 or xi>mx-1: continue
        if xj<0 or xj>mx-1: continue
        if zi+1>mz-1 or yi+1>my-1 or xi+1>mx-1: continue
        if zj+1>mz-1 or yj+1>my-1 or xj+1>mx-1: continue
        if not mask[zi,   yi,   xi  ]: continue
        if not mask[zi+1, yi,   xi  ]: continue
        if not mask[zi,   yi+1, xi  ]: continue
        if not mask[zi+1, yi+1, xi  ]: continue
        if not mask[zi,   yi,   xi+1]: continue
        if not mask[zi+1, yi,   xi+1]: continue
        if not mask[zi,   yi+1, xi+1]: continue
        if not mask[zi+1, yi+1, xi+1]: continue
        if not mask[zj,   yj,   xj  ]: continue
        if not mask[zj+1, yj,   xj  ]: continue
        if not mask[zj,   yj+1, xj  ]: continue
        if not mask[zj+1, yj+1, xj  ]: continue
        if not mask[zj,   yj,   xj+1]: continue
        if not mask[zj+1, yj,   xj+1]: continue
        if not mask[zj,   yj+1, xj+1]: continue
        if not mask[zj+1, yj+1, xj+1]: continue

        i_000 = mask_nonzero_indices_matrix[zi, yi, xi]
        if i_000<0 or i_000>n_x-1: continue
        i_001 = mask_nonzero_indices_matrix[zi, yi, xi+1]
        if i_001<0 or i_001>n_x-1: continue
        i_010 = mask_nonzero_indices_matrix[zi, yi+1, xi]
        if i_010<0 or i_010>n_x-1: continue
        i_011 = mask_nonzero_indices_matrix[zi, yi+1, xi+1]
        if i_011<0 or i_011>n_x-1: continue
        i_100 = mask_nonzero_indices_matrix[zi+1, yi, xi]
        if i_100<0 or i_100>n_x-1: continue
        i_101 = mask_nonzero_indices_matrix[zi+1, yi, xi+1]
        if i_101<0 or i_101>n_x-1: continue
        i_110 = mask_nonzero_indices_matrix[zi+1, yi+1, xi]
        if i_110<0 or i_110>n_x-1: continue
        i_111 = mask_nonzero_indices_matrix[zi+1, yi+1, xi+1]
        if i_111<0 or i_111>n_x-1: continue

        j_000 = mask_nonzero_indices_matrix[zj, yj, xj]
        if j_000<0 or j_000>n_x-1: continue
        j_001 = mask_nonzero_indices_matrix[zj, yj, xj+1]
        if j_001<0 or j_001>n_x-1: continue
        j_010 = mask_nonzero_indices_matrix[zj, yj+1, xj]
        if j_010<0 or j_010>n_x-1: continue
        j_011 = mask_nonzero_indices_matrix[zj, yj+1, xj+1]
        if j_011<0 or j_011>n_x-1: continue
        j_100 = mask_nonzero_indices_matrix[zj+1, yj, xj]
        if j_100<0 or j_100>n_x-1: continue
        j_101 = mask_nonzero_indices_matrix[zj+1, yj, xj+1]
        if j_101<0 or j_101>n_x-1: continue
        j_110 = mask_nonzero_indices_matrix[zj+1, yj+1, xj]
        if j_110<0 or j_110>n_x-1: continue
        j_111 = mask_nonzero_indices_matrix[zj+1, yj+1, xj+1]
        if j_111<0 or j_111>n_x-1: continue

        if abs(zi-zj)<3 or abs(yi-yj)<3 or abs(xi-xj)<3: continue
        ...
        if pair_id in pair_ids: continue
    ...
    def loop_kji(Zi, Yi, Xi, Zj, Yj, Xj, mask, mask_nonzero_indices_zyx_tuple, mask_nonzero_indices_matrix, pair_ids):
      ...
      for mi in range(n_indices):
        ...
        if index_i<0 or index_i>n_indices-1: continue
        index_j = mask_nonzero_indices_matrix[zj, yj, xj]
        if index_j<0 or index_j>n_indices-1: continue
  ...
  with Timer(f"\tbuild csr matrix - {interpolation}", verbose=verbose>10):
    ...
    for pi, p in enumerate(hcsym_pairs):
      ...
      ri = R.from_euler('z', twist_degree * hsym_i + csym_i*360/csym, degrees=True)
      ...
      Zi = tmp_xyz[:,2].reshape((nz, ny, nx))+nz//2 + rise_pixel*hsym_i  # axes order: z, y, x

      rj = R.from_euler('z', twist_degree * hsym_j + csym_j*360/csym, degrees=True)
      ...
      Zj = tmp_xyz[:,2].reshape((nz, ny, nx))+nz//2 + rise_pixel*hsym_j  # axes order: z, y, x
      ...
      csr_row_tmp, csr_col_tmp, csr_data_tmp, row_count_tmp = loop_kji(Zi, Yi, Xi, Zj, Yj, Xj, mask, mask_nonzero_indices_zyx_tuple, mask_nonzero_indices_matrix, pair_ids)
      row_count += row_count_tmp  
      if verbose>20: print(f"{pi+1}/{len(hcsym_pairs)}:", round(angle,2), (hsym_i, csym_i), (hsym_j, csym_j), f"{n_x:,}", f"+{row_count_tmp:,}", f"{row_count:,}", f"target={min_sym_pairs:,}")
      if row_count_tmp:
        csr_A_tmp = csr_matrix((csr_data_tmp, (csr_row_tmp, csr_col_tmp)), shape=(row_count_tmp, n_x), dtype=np.float32)
        csr_A.append(csr_A_tmp)
      if row_count>=min_sym_pairs: break
...
def build_A_data_matrix(image, scale2d_to_3d, twist_degree, rise_pixel, csym, tilt_degree, psi_degree, dy_pixel, reconstruct_diameter_2d_pixel, reconstruct_length_2d_pixel, reconstruct_diameter_3d_pixel, reconstruct_diameter_3d_inner_pixel, reconstruct_length_3d_pixel, min_projection_lines, interpolation, verbose=0):
      ...
      for k in range(nz): # old x axis before back projection
        for j in range(ny): # same y axis before back projection
          ...
          for i in range(nx): # old z axis before back projection
            ...
            if index_000<0 or index_000>n_x-1: continue
            index_001 = mask_nonzero_indices_matrix[zi, yi, xi+1]
            if index_001<0 or index_001>n_x-1: continue
            index_010 = mask_nonzero_indices_matrix[zi, yi+1, xi]
            if index_010<0 or index_010>n_x-1: continue
            index_011 = mask_nonzero_indices_matrix[zi, yi+1, xi+1]
            if index_011<0 or index_011>n_x-1: continue
            index_100 = mask_nonzero_indices_matrix[zi+1, yi, xi]
            if index_100<0 or index_100>n_x-1: continue
            index_101 = mask_nonzero_indices_matrix[zi+1, yi, xi+1]
            if index_101<0 or index_101>n_x-1: continue
            index_110 = mask_nonzero_indices_matrix[zi+1, yi+1, xi]
            if index_110<0 or index_110>n_x-1: continue
            index_111 = mask_nonzero_indices_matrix[zi+1, yi+1, xi+1]
            if index_111<0 or index_111>n_x-1: continue
            has_projection_data = True

            zf = Z[k,j,i] - zi
            yf = Y[k,j,i] - yi
            xf = X[k,j,i] - xi
            for index in (index_000, index_001, index_010, index_011, index_100, index_101, index_110, index_111):
          ...
          if has_projection_data:
            ...
            row_count_tmp += 1
      return csr_row_tmp[:csr_rc_tmp_count], csr_col_tmp[:csr_rc_tmp_count], csr_data_tmp[:csr_rc_tmp_count], b_tmp[:row_count_tmp], b_pid_tmp[:row_count_tmp]
  else:   # nearest neighbor
    @jit(nopython=True, cache=True, nogil=True)
    def loop_kji(Z, Y, X, mask, mask_nonzero_indices_matrix, n_x, pixel_vals):
      nz, ny, nx = Z.shape
      ...
      for k in range(nz): # old x axis before back projection
        for j in range(ny): # same y axis before back projection
          ...
          for i in range(nx): # old z axis before back projection
            ...
            if index<0 or index>n_x-1: continue
            has_projection_data = True
            csr_row_tmp[csr_rc_tmp_count] = row_count_tmp
            csr_col_tmp[csr_rc_tmp_count] = index
            csr_rc_tmp_count += 1
  ...
  for hci, (hi, ci) in enumerate(hcsyms):
    ...
    Z = coords[:,2].reshape((nz, ny, nx)) + reconstruct_length_3d_pixel//2  # axes order: z, y, x
          
    csr_row_tmp, csr_col_tmp, csr_data_tmp, b_tmp, b_pid_tmp = loop_kji(Z, Y, X, mask, mask_nonzero_indices_matrix, n_x, pixel_vals)
    n_b += len(b_tmp)
    if verbose>20: print(f"{hci+1}/{len(hcsyms)}: {hi=} {ci=} +{len(b_tmp):,} {n_b:,} target_lines={min_projection_lines:,}")
    if len(b_tmp):
      ...
      csr_A_tmp = csr_matrix((csr_data_tmp, (csr_row_tmp, csr_col_tmp)), shape=(len(b_tmp), n_x), dtype=np.float32)
      csr_A.append( csr_A_tmp)
      csr_b.append( b_tmp )
      b_pid += [b_pid_tmp]
    if min_projection_lines>0 and n_b>min_projection_lines: break
  from scipy.sparse import vstack  
  A = vstack(csr_A)
...
def back_project_2d_coords_to_3d_coords(image, scale2d_to_3d, reconstruct_diameter_2d_pixel=-1, reconstruct_length_2d_pixel=-1):
  ny, nx = image.shape
  if reconstruct_diameter_2d_pixel<=0: reconstruct_diameter_2d_pixel = ny
  if reconstruct_length_2d_pixel<=0: reconstruct_length_2d_pixel = nx  # direction of helical axis
  ...
  k = np.arange(0, reconstruct_diameter_2d_pixel, dtype=np.int32)-reconstruct_diameter_2d_pixel//2
  j = np.arange(0, reconstruct_diameter_2d_pixel, dtype=np.int32)-reconstruct_diameter_2d_pixel//2
  i = np.arange(0, reconstruct_length_2d_pixel, dtype=np.int32)-reconstruct_length_2d_pixel//2
  region_pixel_vals = image[np.ix_(j+ny//2, i+nx//2)]  # pixel values to be used for lsq solution

  from scipy.spatial.transform import Rotation as R
  r = R.from_euler('y', 90, degrees=True)
  Z, Y, X = np.meshgrid(k.astype(np.float32), j.astype(np.float32), i.astype(np.float32), indexing='ij')
  ...
  X2 = coords[:,0].reshape((reconstruct_diameter_2d_pixel, reconstruct_diameter_2d_pixel, reconstruct_length_2d_pixel))  # axes order: z, y, x
  Y2 = coords[:,1].reshape((reconstruct_diameter_2d_pixel, reconstruct_diameter_2d_pixel, reconstruct_length_2d_pixel))  # axes order: z, y, x
  Z2 = coords[:,2].reshape((reconstruct_diameter_2d_pixel, reconstruct_diameter_2d_pixel, reconstruct_length_2d_pixel))  # axes order: z, y, x
...
def simulate_helical_projection(n, twist, rise, csym, helical_diameter, ball_radius, polymer, planarity, ny, nx, apix, tilt=0, rot=0, psi=0, dy=0):
  ...
  def simulate_projection(centers, sigma, ny, nx, apix):
    ...
    return d
  def helical_unit_positions(n, twist, rise, csym, diameter, height, polymer=0, planarity=1.0, tilt=0, rot=0, psi=0, dy=0):
    assert n>=1
    from scipy.spatial.transform import Rotation as R
    if polymer:
      centers_0 = random_polymer(n_atoms=n, rmin=0, rmax=helical_diameter/2, csym=csym, planarity=planarity)
  ...
  centers = helical_unit_positions(n, twist, rise, csym, helical_diameter, height=nx*apix, polymer=polymer, planarity=planarity, tilt=tilt, rot=rot, psi=psi, dy=dy)
...
def random_polymer(n_atoms=100, rmin=0, rmax=100, csym=1, planarity=0.9):
  ...
  def symmetrize(p, csym=1):
    if csym<=1: return np.expand_dims(p, axis=0)
    from scipy.spatial.transform import Rotation as R
    ret = [p]
  ...
  def are_positions_good(new_points, existing_points, min_dist):
    ...
    if new_points.shape == existing_points.shape and np.allclose(new_points, existing_points):
       dist[np.diag_indices_from(dist)] = 1e10
    if np.any(dist < min_dist):
      return False
    return True
  ...
  while n_trials < max_trials:
    ...
    while ns_trials < max_trials:
      ...
      if are_positions_good(xyz[0:csym, :], xyz[0:csym, :], min_dist=ca_dist*0.8):
    ...
    for i in range(1, n_atoms):
      ...
      while ni_trials < max_trials:
        ...
        p = next_point(step_length=ca_dist, csym=csym, rmin=rmin, rmax=rmax, planarity=planarity, existing_points=existing_points)
...
def estimate_diameter(data):
  ...
  (amp, center, sigma, background), _ = curve_fit(gaussian, x, y, bounds=(lower_bounds, upper_bounds))
...
def sorted_hsym_csym_pairs(twist, rise, csym, nz):
  ...
  for p in hcsym_pairs:
    ...
    hcsym_pair_angles.append((angle, abs(hsym1+hsym2), abs(hsym1-hsym2), abs(hsym1), abs(hsym2), p))
...
def saveMaps(results, target_size, target_apix, mapFilePrefix, verbose=0):
  ...
  for group_key, result_group in result_groups:
    ...
    for ri, result in enumerate(result_group):
      score, (rec3d_x_proj, rec3d_y_proj, rec3d_z_sections, rec3d, reconstruct_diameter_2d_pixel, reconstruct_diameter_3d_pixel, reconstruct_length_2d_pixel, reconstruct_length_3d_pixel), (data, imageFile, imageIndex, apix3d, apix2d, twist, rise, csym, tilt, psi, dy) = result
      mapFilePrefix_tmp = mapFilePrefix + f".{pathlib.Path(imageFile).stem}-{imageIndex}.pitch-{round(rise*360/abs(twist),1)}_twist-{round(twist,3)}_rise-{rise}_csym-{csym}{tilt_psi_dy_str(tilt, psi, dy, sep='_', sep2='-', unit=False)}"
      if rec3d is not None:
        fsc_test = rec3d[1] is not None and rec3d[2] is not None
        for rec3d_i, rec3d_map in enumerate(rec3d):
          if rec3d_map is None: continue
          if len(list(result_group))>1: tag = f".top-{ri+1}"
          else: tag = ""
          if fsc_test:
...
def writePdf(results, pdfFilePrefix, top_k=10, use_pitch=None, image_info="", cmap=None):
  ...
  for (imageFile, imageIndex), group_results in result_groups:
    ...
    for ri, result in enumerate(group_results[:top_k if top_k>0 else None]):
      score, (rec3d_x_proj, rec3d_y_proj, rec3d_z_sections, rec3d, reconstruct_diameter_2d_pixel, reconstruct_diameter_3d_pixel, reconstruct_length_2d_pixel, reconstruct_length_3d_pixel), (data, imageFile, imageIndex, apix3d, apix2d, twist, rise, csym, tilt, psi, dy) = result
      fig = plotOneResult(ri if len(group_results)>1 else -1, score, data, rec3d_y_proj, rec3d_x_proj, rec3d_z_sections, reconstruct_diameter_2d_pixel, reconstruct_diameter_3d_pixel, reconstruct_length_2d_pixel, reconstruct_length_3d_pixel, imageFile, imageIndex, apix3d, apix2d, twist, rise, csym, tilt, psi, dy, image_info, cmap=cmap)
      pdf.attach_note(fig._suptitle.get_text())
      pdf.savefig(fig)
      plt.close(fig)
  ...
  if len(fscs)>1:
    ...
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.02+0.025*nrow), ncol=ncol, fontsize=fontsize)
    pdfFile = f"{pdfFilePrefix}.fsc.pdf"
    pdfFiles.append(pdfFile)
    plt.savefig(pdfFile, format="pdf")
  return pdfFiles

def plotOneResult(rank, score, data, rec3d_y_proj, rec3d_x_proj, rec3d_z_sections, reconstruct_diameter_2d_pixel, reconstruct_diameter_3d_pixel, reconstruct_length_2d_pixel, reconstruct_length_3d_pixel, imageFile, imageIndex, apix3d, apix2d, twist, rise, csym, tilt, psi, dy, image_info, cmap=None):
  ...
  fig.axes[2].set_title("Projection-X", y=-0.1*nx_pad/reconstruct_diameter_2d_pixel)
  fig.axes[3].set_title("Z central section", y=-0.1*nx_pad/reconstruct_diameter_2d_pixel)
  if plot_extra_row:
    ax = fig.add_subplot(gs[2, :])
    ax.imshow(rec3d_x_proj, cmap=cmap)
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
    ax.set_title("Projection-X (length = 1.2 pitch)", y=-0.04*rec3d_x_proj.shape[1]/reconstruct_diameter_2d_pixel)
  plt.tight_layout()
  return fig
...
def plotOneGroupScores(results, image_info, use_pitch=None):
  ...
  for result in results:
    score, _, (_, imageFile, imageIndex, apix3d, apix2d, twist, rise, csym, tilt, psi, dy) = result
    scores.append([twist, rise, tilt, psi, dy, score])
  if use_pitch:
    scores = [[np.round(360/np.abs(twist)*rise, 1), rise, tilt, psi, dy, score] for twist, rise, tilt, psi, dy, score in scores]
  scores.sort()
  ...
  vars = [(n_pts, pts, pts_label), (n_rises, rises, 'rise (Å)'), (n_tilts, tilts, 'tilt (°)'), (n_psis, psis, 'psi (°)'), (n_dys, dys, 'dy (Å)')]
  ...
  for (imageFile, imageIndex), group_results in result_groups:
    ...
    with open(lstFile, "w") as fp:
      ...
      for ri, result in enumerate(group_results):
        score, (rec3d_x_proj, rec3d_y_proj, rec3d_z_sections, _, _, _, _, _), (data, imageFile, imageIndex, apix3d, apix2d, twist, rise, csym, tilt, psi, dy) = result
        imageFilePath = pathlib.Path(imageFile)
        if imageFilePath.is_absolute():
          imageFilePath_final = imageFilePath
        else:
          imageFilePath_final = pathlib.Path(os.path.relpath(str(imageFilePath.absolute()), str(lstFileFolder)))
        ...
        if ri < n:
          ...
          for ii, image in enumerate(images):
            ...
            fname = str(pathlib.Path(fname).resolve().relative_to(pathlib.Path(lstFileFolder).resolve()))
            fp.write(f"0\t{fname}\ttag={tag}\tsrc_image={str(imageFilePath_final)}\tsrc_image_index={imageIndex-1}\tapix={round(apix2d,3)}\tpitch={round(rise*360/abs(twist),1)}\ttwist={round(twist, 3)}\trise={round(rise, 3)}\t{csym=}{tilt_psi_dy_str(tilt, psi, dy, sep=sep, unit=False)}\tscore={round(score, 6)}\n")
        else:
          fp.write(f"0\t{str(imageFilePath_final)}\tsrc_image_index={imageIndex-1}\tpitch={round(rise*360/abs(twist),1)}\ttwist={round(twist, 3)}\trise={round(rise, 3)}\t{csym=}{tilt_psi_dy_str(tilt, psi, dy, sep=sep, unit=False)}\tscore={round(score, 6)}\n")
  return lstFiles

def tilt_psi_dy_str(tilt, psi, dy, sep=' ', sep2='=', unit=True):
  tpy_str = ""
  if tilt: tpy_str += f"{sep}tilt{sep2}{round(tilt, 2)}" + ("°" if unit else "")
...
def add_args(parser):
  ...
  i_parser.add_argument("--i1", type=int, metavar="<n>", nargs="+", help="Which image to process. 1 (not 0) for the first image. RELION convention. default to all images", default=[])
  
  pt_parser = parser.add_mutually_exclusive_group(required=True)
  pt_parser.add_argument("--pitch", dest="pitches", metavar="<Å>", type=float, nargs="+", help="Use this pitch value (unit: Å)", default=[])
  pt_parser.add_argument("--pitch_range", metavar=("<min>", "<max>", "<step>"), type=float, nargs=3, action="append", help="Test pitch values in this range (unit: Å). default to %(default)s", default=[])
  pt_parser.add_argument("--twist", dest="twists", metavar="<°>", type=float, nargs="+", help="Use this twist value (unit: °)", default=[])
  pt_parser.add_argument("--twist_range", metavar=("<min>", "<max>", "<step>"), type=float, nargs=3, action="append", help="Test twist values in this range (unit: °). default to %(default)s", default=[])
  
  rise_parser = parser.add_mutually_exclusive_group(required=True)
  rise_parser.add_argument("--rise", dest="rises", metavar="<Å>", type=float, nargs="+", help="Use this rise value (unit: Å). default to %(default)s", default=[])
  rise_parser.add_argument("--rise_range", metavar=("<min>", "<max>", "<step>"), type=float, nargs=3, action="append", help="Test rise values in this range (unit: Å). default to %(default)s", default=[])
  
  tilt_parser = parser.add_mutually_exclusive_group(required=False)
  tilt_parser.add_argument("--tilt", dest="tilts", metavar="<°>", type=float, nargs="+", help="Use this tilt value (unit: °). default to %(default)s", default=[])
  tilt_parser.add_argument("--tilt_range", metavar=("<min>", "<max>", "<step>"), type=float, nargs=3, action="append", help="Test tilt values in this range (unit: °). default to %(default)s", default=[])

  psi_parser = parser.add_mutually_exclusive_group(required=False)
  psi_parser.add_argument("--psi", dest="psis", metavar="<°>", type=float, nargs="+", help="Use this in-plane rotation value (unit: °). default to %(default)s", default=[])
  psi_parser.add_argument("--psi_range", metavar=("<min>", "<max>", "<step>"), type=float, nargs=3, action="append", help="Test in-plane rotation values in this range (unit: °). default to %(default)s", default=[])
  
  dy_parser = parser.add_mutually_exclusive_group(required=False)
  dy_parser.add_argument("--dy", dest="dys", metavar="<Å>", type=float, nargs="+", help="Use this shift value (unit: Å). default to %(default)s", default=[])
  dy_parser.add_argument("--dy_range", metavar=("<min>", "<max>", "<step>"), type=float, nargs=3, action="append", help="Test shift values in this range (unit: Å). default to %(default)s", default=[])

  parser.add_argument("--csym", type=int, metavar="<n>", help="Rotational symmetry around the helical axis. default: %(default)s", default=1)
  parser.add_argument("--apix", type=float, metavar="<Å>", help="Use this pixel size. A positive value will overwrite the pixel size read from the image file. default to %(default)s", default=-1)

  parser.add_argument("--target_apix2d", type=float, metavar="<Å>", help="Down-scale images to have this pixel size before 3D reconstruction. <=0 -> no down-scaling. default to %(default)s", default=0)
  parser.add_argument("--target_apix3d", type=float, metavar="<Å>", help="Pixel size of 3D reconstruction. 0 -> Set to target_apix2d. <0 -> auto-decision. default to %(default)s", default=-1)

  parser.add_argument("--fsc", dest="fsc_test", type=int, metavar="<0|1>", help="Preform Fourier Shell Correlation test. 0-no test, 1-random split of input image pixels (recommended), 2-eve/odd split of input image pixels, 3-first/second half split of input image pixels. default: %(default)s", default=0)
  parser.add_argument("--tube_diameter", type=float, metavar="<Å>", help="The outer diameter (Å) of a tube mask to be applied to the 2D image/3D reconstruction. Automatically estimated if <0. default to %(default)s", default=-1)
  parser.add_argument("--tube_diameter_inner", type=float, metavar="<Å>", help="The inner diameter (Å) of a tube mask to be applied to the 3D reconstruction. default to  %(default)s", default=0)
  parser.add_argument("--tube_length", type=float, metavar="<Å>", help="The length (Å) of a tubular mask to be applied to the 2D image. Automatically estimated if <0. default to %(default)s", default=-1)

  rec3d_length_parser = parser.add_mutually_exclusive_group(required=False)
  rec3d_length_parser.add_argument("--reconstruct_length", type=float, metavar="<Å>", help="The length of the 3D reconstruction will be max(this_value, rise) when >0 or 4*rise when <=0. default: %(default)s", default=-1)
  rec3d_length_parser.add_argument("--reconstruct_length_pitch", type=float, metavar="<val>", help="The length of the 3D reconstruction will be max(this_value * pitch, rise) when >0 or 4*rise when <=0. default: %(default)s", default=-1)
  rec3d_length_parser.add_argument("--reconstruct_length_rise", type=float, metavar="<val>", help="The length of the 3D reconstruction will be max(this_value * rise, rise) when >0 or 4*rise when <=0. default: %(default)s", default=-1)
  
  choices = ["tv", "nl_mean", "wavelet"]
  parser.add_argument("--denoise", type=str, metavar=f"<{'|'.join(choices)}>", choices=choices, help="denoise the input images(s). disabled by default", default="")
  parser.add_argument("--low_pass", type=float, metavar="<Å>", help="Low pass filter of the input image(s) before further analysis. disabled by default", default=-1)
  parser.add_argument("--transpose", type=int, metavar="<-1|0|1>", help="Transpose the image to change the helical structure from vertical direction (cryoSPARC) to horizontal direction (RELION). Use a negative value to enable auto-detection. default to %(default)s", default=-1)
  parser.add_argument("--horizontalize", type=int, metavar="<0|1>", help="Automatically rotate the helical structure to horizontal direction (e.g. the helical axis along X-axis). default to %(default)s", default=1)
  parser.add_argument("--thresh_fraction", type=float, metavar="[0, 1)", help="Threshold the image (threshold=image_max*thresh_fraction) before analysis. Use a negative value to disable thresholding. default to %(default)s", default=0)
  parser.add_argument("--positive_constraint", type=int, metavar="<-1|0|1>", help="Enforce positive constraint for the reconstruction. Use a negative value for automatic decision. default to %(default)s", default=0)
  parser.add_argument("--sym_oversample", type=int, metavar="<n>", help="Helical sym and csym oversampling factor that controls the number of equations when setting up the A matrix of the least square solution. larger values (e.g. 100) -> slower but better quality. A negative value means auto-decision. default: %(default)s", default=-1)
  choices = "linear linear11 linear01 linear10 nn".split()  # linear = linear11
  parser.add_argument("--interpolation", type=str, choices=choices, metavar=f"<{'|'.join(choices)}>", help="Choose the interpolation method. default: %(default)s", default="linear")
  choices = "elasticnet lasso ridge lreg lsq".split()
  parser.add_argument("--algorithm", type=str, choices=choices, metavar=f"<{'|'.join(choices)}>", help="Choose the algorithm that will be used to solve the equations. default: %(default)s", default="elasticnet")
  parser.add_argument("--alpha", type=float, metavar="<val>", help="Weight of regularization. Only used for elasticnet, lasso and ridge algorithms. default: 1e-4 for elasticnet/lasso and 1 for ridge", default=-1)
  parser.add_argument("--l1_ratio", type=float, metavar="[0,1]", help="The ratio (0 to 1) of L1 regularization in the L1/L2 combined regularization. Only used for the elasticnet algorithms. default: %(default)s", default=0.5)
  parser.add_argument("--top_k", type=int, metavar="<n>", help="Output the largest k solutions. Use a negative value to output all solutions. default: %(default)s", default=10)
  parser.add_argument("--color_map", type=str, metavar="<viridis|gray|etc>", help="Use this color map for image plots. Valid choices are available at https://matplotlib.org/stable/users/explain/colors/colormaps.html. default: %(default)s", default="gray")
  parser.add_argument("--save_projections", type=int, metavar="<0|1>", help="Save x/y/z projection images of the reconstructed 3D map. default: %(default)s", default=0)
  parser.add_argument("--cpu", type=int, metavar="<n>", help="Use this number of cpus. A negative value means all currently unused cpus. default: %(default)s", default=-1)
  parser.add_argument("--force", type=int, metavar="<0|1>", help="Clear the cached results and re-compute the reconstruction(s). default: %(default)s", default=0)
  parser.add_argument("--verbose", type=int, metavar="<n>", help="Verbose level. default: %(default)s", default=2)
...
def check_args(args, parser):
  if len(args.pitches):
    ...
    tmp = [np.arange(start, end+step/2, step) for start, end, step in args.pitch_range]
    ...
    tmp = [np.arange(start, end+step/2, step) for start, end, step in args.twist_range]
    args.twists = np.sort(np.unique(np.concatenate(tmp)))
    args.pitches = []
  else:
    print(f"ERROR: --pitch, --pitch_range, --twist, or --twist_range must be specified")
    sys.exit(-1)

  if len(args.rises):
    args.rises = np.array(args.rises)
  elif len(args.rise_range):
    tmp = [np.arange(start, end+step/2, step) for start, end, step in args.rise_range]
  ...
  if len(args.tilts):
    ...
    tmp = [np.arange(start, end+step/2, step) for start, end, step in args.tilt_range]
  ...
  if len(args.psis):
    ...
    tmp = [np.arange(start, end+step/2, step) for start, end, step in args.psi_range]
  ...
  if len(args.dys):
    ...
    tmp = [np.arange(start, end+step/2, step) for start, end, step in args.dy_range]
    args.dys = np.sort(np.unique(np.concatenate(tmp)))
  else:
    args.dys = np.array([0.0])

  args.i = []
  if len(args.i1): args.i = sorted(list(set(args.i1)))
  if len(args.i0): args.i = sorted([i+1 for i in set(args.i0)])

  args.l1_ratio = max(0, min(1, args.l1_ratio))
  args.algorithm = dict(model=args.algorithm, l1_ratio=args.l1_ratio)
  if args.alpha>=0: args.algorithm["alpha"] = args.alpha

  if args.output_prefix.find("/")==-1 or args.output_prefix[-1] == "/":
    args.output_prefix = str(pathlib.Path(__file__).stem.upper()) + "/" + args.output_prefix.rstrip("/")
  
  if args.csym>parser.get_default('csym'): args.output_prefix += f".csym-{args.csym}"
  if args.tube_diameter>0: args.output_prefix += f".diameter-{args.tube_diameter}"
  if args.tube_length>0: args.output_prefix += f".length2d-{args.tube_length}"
  if args.reconstruct_length>0: args.output_prefix += f".length3d-{args.reconstruct_length}A"
  if args.reconstruct_length_pitch>0: args.output_prefix += f".length3d-{args.reconstruct_length_pitch}pitch"
  if args.reconstruct_length_rise>0: args.output_prefix += f".length3d-{args.reconstruct_length_rise}rise"
  if args.denoise!=parser.get_default('denoise'): args.output_prefix += f".denoise-{args.denoise}"
  if args.low_pass!=parser.get_default('low_pass'): args.output_prefix += f".low_pass-{args.low_pass}"
  if args.horizontalize!=parser.get_default('horizontalize'): args.output_prefix += f".horizontalize-{args.horizontalize}"
  if args.positive_constraint!=parser.get_default('positive_constraint'): args.output_prefix += f".positive-{args.positive_constraint}"
  if args.target_apix3d!=parser.get_default('target_apix3d'): args.output_prefix += f".apix3d-{args.target_apix3d}"
  if args.target_apix2d!=parser.get_default('target_apix2d'): args.output_prefix += f".apix2d-{args.target_apix2d}"
  if args.thresh_fraction>0: args.output_prefix += f".thresh-{args.thresh_fraction}"
  if args.sym_oversample!=parser.get_default('sym_oversample'): args.output_prefix += f".sym_oversample-{args.sym_oversample}"
  if args.interpolation!=parser.get_default('interpolation'): args.output_prefix += f".interpolation-{args.interpolation}"
  if args.algorithm["model"]!=parser.get_default('algorithm'): args.output_prefix += f".algorithm-{args.algorithm['model']}"
  if args.alpha!=parser.get_default('alpha'): args.output_prefix += f".alpha-{args.alpha}"
  if args.l1_ratio!=parser.get_default('l1_ratio'): args.output_prefix += f".l1_ratio-{args.l1_ratio}"
