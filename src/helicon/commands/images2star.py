...
def main(args):
    ...
    if args.verbose:
        ...
        if "rlnHelicalTubeID" in data:
            ...
            dist_seg_median, dist_seg_mean, dist_seg_sigma, n_all = estimate_inter_segment_distance(data)
            if dist_seg_median is None:
                print(f"Read in {len(data)} segments in {nHelices} helices from {nMicrographs} micrographs in {len(args.input_imageFiles)} image files{apixStr}")
            else:
                print(f"Read in {len(data):,} segments (extracted with {dist_seg_median:.2f}Å inter-segment shift) in {nHelices:,} helices from {nMicrographs:,} micrographs in {len(args.input_imageFiles)} image files{apixStr}. Segment distances: {dist_seg_mean:.2f}±{dist_seg_sigma:.2f}Å. Estimate: ~{len(data)/n_all*100:.1f}% of all (~{n_all:,}) segments")
                if dist_seg_sigma>dist_seg_median:
                    helicon.color_print(f"It appears that the filaments are badly fragmented, probably from Select2D/Select3D jobs. You can avoid filament fragmentation by runing the following command:\nimages2star.py <input.star> <output.star> --recoverFullFilaments minFraction=<0.5>[:forcePickJob=<0|1>][:fullStarFile=<filename>]\nafter each Select2D/Select3D job")
        elif "rlnMicrographMovieName" in data and "rlnMicrographName" not in data and "rlnImageName" not in data:
            print(f"Read in {nMicrographs} movies from {len(args.input_imageFiles)} files{apixStr}")
        elif "rlnMicrographName" in data and "rlnImageName" not in data:
            print(f"Read in {nMicrographs} micrographs from {len(args.input_imageFiles)} files{apixStr}")
        else:
            print(f"Read in {len(data)} particles in {nMicrographs} micrographs from {len(args.input_imageFiles)} image files{apixStr}")
        if tmpCol in data:
            data.drop(tmpCol, inplace=True, axis=1)

    if len(data) == 0:
        helicon.color_print("WARNING: nothing to do with 0 particles. I am going to quit")
        sys.exit(-1)
    ...
    for option_name in args.all_options:
        ...
        if option_name == "removeDuplicates" and param:
            ...
            if len(vars)<1:
                ...
                if args.verbose:
                    print(f"{len(data2)} image retained after removing {len(data) - len(data2)} images with duplicate {vars}")
        ...
        if option_name == "minDuplicates" and param>0:
            ...
            for a in 'rlnImageName rlnMicrographName'.split():
                if a in data:
                    ...
                    break
            if attr is None:
                helicon.color_print(f"\tERROR: required parameter (rlnImageName or rlnMicrographName) is not available")
                sys.exit(-1)
            ...
            assert missing_attrs == [], f"\tERROR: requried parameters {' '.join(missing_attrs)} are not available"
            ...
            def get_input_star_file(starFile, arg="--i "):
                ...
                if not noteFile.exists(): return None
            ...
            if fullStarFile is None:
                def trace_back_to_extract_job(inputStarFile, forcePickJob=0, history=[]):
                    ...
                    if new_inputStarFile is None: return None
                    if new_inputStarFile.find("Polish") != -1 or new_inputStarFile.find("Extract") != -1:
                        if not forcePickJob:
                            history.append(new_inputStarFile)
                            return new_inputStarFile
                        parent_job_pick = get_input_star_file(new_inputStarFile, arg="--coord_list ")
                        parent_job_reextract = get_input_star_file(new_inputStarFile, arg="--reextract_data_star ")
                        if parent_job_pick and parent_job_pick.find("Pick")!=-1:
                            history.append(new_inputStarFile)
                            return new_inputStarFile
                        if parent_job_reextract:
                            history.append(new_inputStarFile)
                            return trace_back_to_extract_job(parent_job_reextract, forcePickJob, history)                          
                    return trace_back_to_extract_job(new_inputStarFile, forcePickJob, history)

                history = []
                fullStarFile = trace_back_to_extract_job(args.input_imageFiles[0], forcePickJob, history)
                if args.verbose>2:
                    tmp = '\t->\n\t'.join(history)
                    print(f"\t{tmp}")
                if fullStarFile is None:
                    ...
                    if len(history)>1:
                        helicon.color_print(f"WARNING: auto-traced back to '{fullStarFile}' but it is not the starting Polish shiny.star or Extract particles.star file. Will use it for recovery but you can manually specify the starting star file with --recoverFullFilaments fullStarFile=<filename>")

                    else:
                        helicon.color_print(f"WARNING: failed to auto-find the Polish shiny.star or Extract particles.star file. Please manually specify it with --recoverFullFilaments fullStarFile=<filename>")
                        sys.exit(-1)
                if args.verbose>2:
                    print(f"\tWill use {str(fullStarFile)} to provide the full filaments")

            parent_job_pick = get_input_star_file(fullStarFile, arg="--coord_list ")
            parent_job_reextract = get_input_star_file(fullStarFile, arg="--reextract_data_star ")
            if parent_job_pick is None and parent_job_reextract:
                helicon.color_print(f"\tWarning: the source of the 'full' filaments\n\t{parent_job_reextract}\n\tis not a ManualPick or AutoPick job. The output star file will probably still be fragmented")

            data.convention = "relion"
            data = data.drop_duplicates(subset=['rlnImageName'], keep='last', inplace=False).reset_index(drop=True)

            data2 = helicon.images2dataframe(fullStarFile, alternative_folders=args.folder, ignore_bad_particle_path=args.ignoreBadParticlePath, ignore_bad_micrograph_path=args.ignoreBadMicrographPath, warn_missing_ctf=0, target_convention="relion")
            data2 = data2.drop_duplicates(subset=['rlnImageName'], keep='last', inplace=False).reset_index(drop=True)
            if args.verbose>1:
                print(f"\tRead in {len(data2):,} particles from {fullStarFile}")

            missing_attrs = [p for p in required_attrs if p not in data2]
            assert missing_attrs == [], f"\tERROR: {fullStarFile} does not have the requried parameters {' '.join(missing_attrs)}"
            ...
            from helicon import convert_dataframe_file_path
            if forcePickJob:
                data.loc[:, "rlnMicrographName_abs"] = convert_dataframe_file_path(data, "rlnMicrographName", to="abs")
                data2.loc[:, "rlnMicrographName_abs"] = convert_dataframe_file_path(data2, "rlnMicrographName", to="abs")
                if not (set(data['rlnMicrographName_abs']).issubset(set(data2['rlnMicrographName_abs']))):
                    missing_micrographs = '\n\t'.join(sorted([m for m in set(data['rlnMicrographName_abs']) if m not in set(data2['rlnMicrographName_abs'])]))
                    helicon.color_print(f"\tERROR: --recoverFullFilament option requires that {fullStarFile} contains identical set or a superset of micrographs. These micrographs are not in {fullStarFile}:\t{missing_micrographs}")

                    sys.exit(-1)
                sortby = ["rlnMicrographName_abs", "rlnHelicalTubeID", "rlnHelicalTrackLengthAngst"]
                ...
                mgraphs_dict = {mgraph_name:mgraph_particles for mgraph_name, mgraph_particles in mgraphs}
                mgraphs2_dict = {mgraph_name:mgraph_particles for mgraph_name, mgraph_particles in mgraphs2}

                def on_line_segment(px, py, line_start_x, line_start_y, line_end_x, line_end_y, epsilon=1.0):
                    d1 = np.sqrt((px-line_start_x)**2+(py-line_start_y)**2)
                    d2 = np.sqrt((px-line_end_x)**2+(py-line_end_y)**2)
                    d = np.sqrt((line_end_x-line_start_x)**2+(line_end_y-line_start_y)**2)
                    return abs(d-d1-d2)<epsilon

                for mgraph_name in mgraphs_dict:
                    filaments = mgraphs_dict[mgraph_name].groupby('rlnHelicalTubeID', sort=False)
                    if mgraph_name not in mgraphs2_dict:
                        helicon.color_print("\tERROR: micrograph {mgraph_name} is not in {fullStarFile}")
                    filaments2 = mgraphs2_dict[mgraph_name].groupby('rlnHelicalTubeID', sort=False)
                    for filament_name, filament_segments in filaments:
                        matched = False
                        n = len(filament_segments)
                        cx0, cx1 = filament_segments["rlnCoordinateX"].astype(np.float32).values[[0, -1]]
                        cy0, cy1 = filament_segments["rlnCoordinateY"].astype(np.float32).values[[0, -1]]
                        for filament_name2, filament_segments2 in filaments2:
                            cx0_2, cx1_2 = filament_segments2["rlnCoordinateX"].astype(np.float32).values[[0, -1]]
                            cy0_2, cy1_2 = filament_segments2["rlnCoordinateY"].astype(np.float32).values[[0, -1]]
                            if on_line_segment(cx0, cy0, cx0_2, cy0_2, cx1_2, cy1_2) and on_line_segment(cx1, cy1, cx0_2, cy0_2, cx1_2, cy1_2):
                                matched = True
                                n2 = len(filament_segments2)
                                indices = list(filament_segments2.index)
                                helices.append((n, n2, indices))
                        if not matched:
                            helicon.color_print("\tWarning: {mgraph_name}:helicalTubeID={helix_name}: cannot find a matching helix in {fullStarFile}")
            else:
                if not (set(data[['rlnImageName']]).issubset(set(data2[['rlnImageName']]))):
                    helicon.color_print(f"\tERROR: --recoverFullFilament option requires that {fullStarFile} contains identical set or a superset of particles")
                    sys.exit(-1)
                
                data.loc[:, "rlnImageName_abs"] = convert_dataframe_file_path(data, "rlnImageName", to="abs").str.split("@", expand=True).iloc[:, -1]
                data2.loc[:, "rlnImageName_abs"] = convert_dataframe_file_path(data2, "rlnImageName", to="abs").str.split("@", expand=True).iloc[:, -1]
                groups  = data.groupby(['rlnImageName_abs', 'rlnHelicalTubeID'], sort=False)
                groups2 = data2.groupby(['rlnImageName_abs', 'rlnHelicalTubeID'], sort=False)
                groups_dict = {group_name:group_particles for group_name, group_particles in groups}
                groups2_dict = {group_name:group_particles for group_name, group_particles in groups2}
                missing_helices = [f'{k[0]}:rlnHelicalTubeID={k[1]}' for k in groups_dict if k not in groups2_dict]
                if missing_helices:
                    s = '\n\t'.join(missing_helices)
                    helicon.color_print(f"\tERROR: {len(missing_helices)} helices not found in {fullStarFile}:\n\t{s}")
                    helicon.color_print(f"\tMake sure that the input star file {' '.join(args.input_imageFiles)} and the fullStarFile {fullStarFile} are from the same Extract job")
                    sys.exit(-1)
            ...
            for n1, n2, helix_indices in helices:
                ...
                if n1/n2 < minFraction: continue
                indices += helix_indices
            data = data2.iloc[indices]
            drop_attrs = [attr for attr in "rlnImageName_abs rlnMicrographName_abs".split() if attr in data]
            if drop_attrs:
            ...
            if args.verbose>1:
                ...
                if folder_current != folder_new:
                    helicon.color_print(f"\tWarning: the output star file now points to particles in folder\n\t{folder_new}\n\tinstead of\n\t{folder_current}")
            if args.verbose>2:
                import matplotlib.pyplot as plt
                fig, axes = plt.subplots(nrows=2, ncols=1, figsize=(6, 8))
                hbin = axes[0].hexbin(nsegments, fractions, bins='log', gridsize=50, cmap='jet')
            ...
            if var in data:
                ...
                if len(data)<1:
                    helicon.color_print(f"WARNING: this selection has excluded all images. Hint: the actual data range is [{vmin}, {vmax}]")
                    sys.exit(-1)
            else:
                if args.verbose:
                    helicon.color_print("\tWARNING: the variable \"%s\" specified by option \"--select %s %s\" does NOT exist" % (
                        var, var, val))
            index_d[option_name] += 1
            ...
            if var in data:
                ...
                if args.verbose:
                    helicon.color_print("\tWARNING: the variable \"%s\" specified by option \"--selectValueRange %s %s %s\" does NOT exist" % (
                        var, var, val1, val2))
            index_d[option_name] += 1
            ...
            if var in data:
                ...
                if pd.api.types.is_integer_dtype(data[var]):
                    ...
                    if args.verbose:
                        helicon.color_print("ERROR: the variable \"%s\" specified by option \"--selectRatioRange %s %s %s\" is NOT a number type" % (
                        var, var, val1, val2))
                        sys.exit(-1)
                ...
                if val1 < 0: val1 = 0
                if val2 < 0: val2 = len(data)
                data = data.iloc[val1:val2]
            else:
                if args.verbose:
                    helicon.color_print("\tERROR: the variable \"%s\" specified by option \"--selectRatioRange %s %s %s\" does NOT exist" % (
                        var, var, val1, val2))
                    sys.exit(-1)
            if args.verbose > 1:
            ...
            if not len(data):
                if args.verbose:
                    print("\tNothing to do when there is no particle image left. I will quit")
                sys.exit(-1)
            index_d[option_name] += 1
            ...
            if not os.path.exists(sf):
                helicon.color_print("\tERROR: option --selectFile=%s has specified a non-existent file %s" % (args.selectFile, sf))
                sys.exit(-1)
            ...
            if len(data2):
                ...
                inputFileStr = args.input_imageFiles if len(args.input_imageFiles)>1 else args.input_imageFiles[0]
                if option_name in ["selectFile"]:
                    helicon.color_print(("\tERROR: no common image found. Check if the files %s and %s include particles in the same folder" % (inputFileStr, sf)))
            ...
            assert missing_attrs == [], f"\tERROR: requried parameters {' '.join(missing_attrs)} are not available in {sf}"

            from helicon import convert_dataframe_file_path
            data["sbpl_rlnMicrographName"] = convert_dataframe_file_path(data, "rlnMicrographName", to="abs")
            data_sf["sbpl_rlnMicrographName"] = convert_dataframe_file_path(data_sf, "rlnMicrographName", to="abs")
            ...
            for gname, g in data.groupby("sbpl_rlnMicrographName"):
                if gname not in group2: continue
            ...
            if nkeep < 1: nkeep = elbow

            if args.verbose > 1:
                print("\tNumber of particles in %d Euler groups: Mean=%.1f\tSigma=%.1f\tMedian=%d\tElbow=%d" % (len(binAssignments), counts.mean(), counts.std(), counts.median(), elbow))
                print("\tUpto %d best particles in each angular bin will be retained" % (nkeep))
            ...
            for bi, be in enumerate(binEulers):
                ...
                if "rlnLogLikeliContribution" in binPtcls:
                    binPtcls = binPtcls.sort_values("rlnLogLikeliContribution", ascending=1)  # the larger rlnLogLikeliContribution the better
                    ...
                    if len(binPtcls) > nkeep:
                        ...
                        binPtcls2 = binPtcls
                if args.verbose > 2:
                    print("\t%3d/%3d:\talt=%.3g\taz=%.3g\t%d\t->\t%d" % (bi + 1, len(binEulers), be[0], be[1], len(binPtcls), len(binPtcls2)))
            ...
            if len(param) % 2:
                helicon.color_print("ERROR: you specified odd number of --setParm arguments. Only even number of arguments are allowed for var val pairs")
                sys.exit(1)
        ...
        elif option_name == "setBeamTiltClass" and param>0:   # one group per micrograph
            ...
            if var not in data:
                helicon.color_print(f"ERROR: parameter {var} does not exist. Cannot add a value to it")
                sys.exist(-1)
            data1 = data
            data2 = data1.copy()
            data2.loc[:, var] += 180.0
            ...
            var, val = param
            if var not in data:
                helicon.color_print(f"ERROR: parameter {var} does not exist. Cannot multiply it by another value")
            data[var] *= float(val)
            index_d[option_name] += 1
            ...
            for var_from, var_to in param:
                if var_from not in data:
                    helicon.color_print(("\tWARNING: %s does not exist. Cannot duplicate %s to %s" % (var_from, var_from, var_to)))
                    continue
                if var_to in data:
                    helicon.color_print(("\tWARNING: %s already exists. Will not duplicating %s to %s" % (var_to, var_from, var_to)))
                    continue
            ...
            for parm in zip(*[iter(param)] * 2):
                ...
                if var_old not in data:
                    helicon.color_print(("\tWARNING: %s does not exist. Cannot rename %s to %s" % (var_old, var_old, var_new)))
                    continue
                if var_new in data:
                    helicon.color_print(("\tWARNING: %s already exists. Cannot duplicate %s to %s" % (var_new, var_old, var_new)))
                    continue
            ...
            for p in param:
                p = p.strip('_')
                if p in data: dropParms.append(p)
                else: invalidParms.append(p)
            if invalidParms:
        ...
        # copy parameters in the other star file
            ...
            data2 = helicon.images2dataframe(starFile, alternative_folders=args.folder, ignore_bad_particle_path=args.ignoreBadParticlePath, ignore_bad_micrograph_path=args.ignoreBadMicrographPath, warn_missing_ctf=0, target_convention="relion")
            data2 = data2.drop_duplicates(subset=['rlnImageName'], keep='last', inplace=False)
            if args.verbose>1:
                print(("\tRead in %d particles from %s" % (len(data2), starFile)))

            if len(data) > len(data2):
                helicon.color_print(("\tERROR: --copyParm option requires that %s (%d) has the same number or more particles (>=%d needed)" % (
                starFile, len(data2), len(data))))
                sys.exit(-1)

            if not (set(data[['rlnImageName']]).issubset(set(data2[['rlnImageName']]))):
                helicon.color_print(("\tERROR: --copyParm option requires that %s contains identical set or a superset of particles" % (
                starFile)))
                sys.exit(-1)
            ...
            if len(vars):
                ...
                if len(copyVars):
                    ...
                    if len(invalidParms):
                        helicon.color_print(("\tWARNING: parameters \"%s\" not in the list of parameters %s from file %s. ignored" % (' '.join(invalidParms), list(data2.columns), starFile)))
            ...
            from helicon import convert_dataframe_file_path
            data.loc[:, "rlnImageName_abs"] = convert_dataframe_file_path(data, "rlnImageName", to="abs")
            data2.loc[:, "rlnImageName_abs"] = convert_dataframe_file_path(data2, "rlnImageName", to="abs")
            ...
            if var in data:
                ...
                helicon.color_print(("\tWARNING: variable %s does not exist. Skipped" % (var)))
            index_d[option_name] += 1
            ...
            if nImage != len(data):
                helicon.color_print(f"\tERROR: {replaceImageName} contains {len(nImage)} particles, different from the expected {len(data)} particles")
                sys.exit(-1)

            data["rlnImageName"] = pd.Series(list(range(1, nImage+1))).map('{:06d}'.format) + "@" + replaceImageName
            index_d[option_name] += 1
            ...
            def setMicrographCTF(mgraphName, mgraphParticles, data, ctfparms):
                ...
                if mid in ctfparms:
                    ...
                    helicon.color_print("\tERROR: cannot find ctf parmeters for micrograph %s" % (mgraphName))
                    sys.exit(1)

                data.loc[mgraphParticles.index, "rlnVoltage"] = d["voltage"]
                data.loc[mgraphParticles.index, "rlnSphericalAberration"] = d["cs"]
                data.loc[mgraphParticles.index, "rlnAmplitudeContrast"] = d["ampcont"] / 100.  # [0, 100] -> [0, 1]
                data.loc[mgraphParticles.index, "rlnMagnification"] = data.loc[
                                                                          mgraphParticles.index, "rlnDetectorPixelSize"] * 1e4 / \
                                                                      d["apix"]

                rlnDefocusU, rlnDefocusV, rlnDefocusAngle = helicon.eman_astigmatism_to_relion(d["defocus"], d["dfdiff"],
                                                                                             d["dfang"])
            ...
            data2 = helicon.images2dataframe(targetStarFile, alternative_folders=args.folder, ignore_bad_particle_path=args.ignoreBadParticlePath, ignore_bad_micrograph_path=args.ignoreBadMicrographPath, warn_missing_ctf=1, target_convention="relion")
            data2 = data2.drop_duplicates(subset=['rlnImageName'], keep='last', inplace=False)
            ...
            data2['astig_x'] = data2['delta_defocus']*np.cos(np.deg2rad(data2['rlnDefocusAngle']))
            data2['astig_y'] = data2['delta_defocus']*np.sin(np.deg2rad(data2['rlnDefocusAngle']))
            data2 = data2.groupby('rlnMicrographName').mean()
            data2['mean_astig'] = np.sqrt(data2['astig_x']**2+data2['astig_y']**2)
            data2['mean_astig_angle'] = np.arctan2(data2['astig_y'],data2['astig_x']) * 180 / np.pi
            for micrograph in data2.index:
                if micrograph in data['rlnMicrographName'].values:
                    ...
                    data.loc[micrograph_rows,'rlnDefocusU']=data2.loc[micrograph,'mean_defocus']+data2.loc[micrograph,'mean_astig']
                    data.loc[micrograph_rows,'rlnDefocusV']=data2.loc[micrograph,'mean_defocus']-data2.loc[micrograph,'mean_astig']
                    data.loc[micrograph_rows,['rlnDefocusAngle','rlnCtfBfactor','rlnCtfScalefactor','rlnPhaseShift']]=data2.loc[micrograph,['mean_astig_angle','rlnCtfBfactor','rlnCtfScalefactor','rlnPhaseShift']].values
            ...
            if badParms:
                if len(badParms)>1:
                    helicon.color_print("\tERROR: parameters %s do not exist" % (" ".join(badParms)))
                else:
                    helicon.color_print("\tERROR: parameter %s does not exist" % (badParms[0]))
                sys.exit(-1)
            ...
            for attr in "rlnImageName rlnMicrographName".split():
                if attr in data:
                    ...
                    relion_proj_folder = get_relion_project_folder(os.path.abspath(args.output_starFile))
                    relpath_start = os.path.dirname(os.path.abspath(args.output_starFile)) if relion_proj_folder is None else relion_proj_folder
                    data[attr] = convert_dataframe_file_path(data, attr, to=path, relpath_start=relpath_start)
            index_d[option_name] += 1
            ...
            for mgraphName, mgraphParticles in mgraphs:
                ...
                if not (os.path.exists(mgraphName2) and helicon.EMUtil.get_image_count(mgraphName2) == n):
                    particles_indices = sorted(list(indices.iloc[mgraphParticles.index].astype(int)))
                    for i in range(n):
                        i2 = particles_indices[i]-1
                        d.read_image(mgraphName, i2)
                        d.write_image(mgraphName2, i)
                rlnImageName = pd.Series(list(range(1, n+1))).map('{:06d}'.format) + '@' + mgraphName2
                data.loc[mgraphParticles.index, "rlnImageName"] = rlnImageName.values   # critical to use values, otherwise the index-aligned assignment will mess up the values
                count += 1
                if args.verbose:
                    print("\t%d/%d: %d/%d images in %s saved to %s" % (count, len(mgraphs), len(mgraphParticles), helicon.EMUtil.get_image_count(mgraphName), mgraphName, mgraphName2))
            index_d[option_name] += 1

        elif option_name == "fullStack" and param:
            valid_cols = set("rlnVoltage rlnDefocusU rlnDefocusV rlnDefocusAngle rlnSphericalAberration rlnDetectorPixelSize rlnMagnification rlnAmplitudeContrast rlnMicrographName rlnGroupName rlnGroupNumber".split())
            ...
            for mgraphName, mgraphParticles in mgraphs:
                ...
                rlnImageName = pd.Series(list(range(1, n+1))).map('{:06d}'.format) + '@' + mgraphName
                df = pd.DataFrame()
                ...
                count += 1
                if args.verbose:
                    print("\t%d/%d: %s:\t%d -> %d images" % (count, len(mgraphs), mgraphName, len(mgraphParticles), n))
            ...
            if not force:
                if os.path.exists(outputFile):
                    with mrcfile.open(outputFile, header_only=True) as mrc:
                        if not (mrc.header.nx == newsize and mrc.header.ny == newsize and mrc.header.nz == nImage):
                            force = 1
                else:
                    force = 1
            if force:
                from tqdm import tqdm
                if float16: mrc_mode = 12
                else: mrc_mode = 2
                with mrcfile.new_mmap(outputFile, shape=(nImage, newsize, newsize), mrc_mode=mrc_mode, fill=None, overwrite=True) as mrc:
                    apix0 = None
                    for i in tqdm(list(range(nImage)), unit=' particles', disable=args.verbose>1):
                        if args.verbose > 1:
                            print("\t%d/%d: adding %s:%d" % (i+1, nImage, images["filename"].iloc[i], images["pid"].iloc[i]))
                        d = helicon.read_image_2d(images["filename"].iloc[i], int(images["pid"].iloc[i]-1))
                        if apix0 is None: apix0 = d["apix_x"]
                        if newsize<nx:
                            d = d.FourTruncate(newsize, newsize, 1, 1)  # crop Fourier transform
                        elif newsize>nx:
                            d = d.FourInterpol(newsize, newsize, 1, 1)  # pad Fourier transform
            ...
            if optics is not None and newsize != nx:
                ...
                if "rlnImagePixelSize" in optics:
                    optics.loc[:, "rlnImagePixelSize"] = optics.loc[:, "rlnImagePixelSize"] * nx/newsize
            index_d[option_name] += 1
            ...
            supported_standards=dict(graphene=2.13, graphene_oxide=2.13, go=2.13, gold=2.355, ice=3.661)   # unit: Angstrom
            target_res = supported_standards[standard_sample.lower()]
            apix, pixelSize_source = getPixelSize(data, return_pixelSize_source=True)
            if apix is None:
                helicon.color_print('\tERROR: cannot find "rlnImagePixelSize" or "rlnMicrographPixelSize"')
                sys.exit(-1)
            half_corner_res = 1./(1/(2*apix)*(1 + np.sqrt(2))/2)
            if target_res<=half_corner_res:
                helicon.color_print(f"\tERROR: target resolution {target_res} Å for {param} is beyond the limit ({half_corner_res:.2f} Å = (1+sqrt(2))/2 * Nyquist resolution)")
                sys.exit(-1)
            ...
            theta_samples = int(np.pi/((1/res_high-1/res_low)/(r_samples-1)/(1/target_res)))+1  # equal sampling in radial and angular directions
            if args.verbose>1:
                if args.verbose>2:
                    print(f"\tCurrent {pixelSize_source}: {apix} Å (Nyquist={2*apix} Å)")
                print(f"\tResolution range (±{search_range*100}%) to search for diffraction peak ({target_res} Å) of {param}: {res_low:.2f} -> {res_high:.2f} Å")

            def fft_resolution_range(images, apix, res_low=0, res_high=0, r_samples=-1, theta_samples=180, return_R_only=False):
                import finufft
                import numpy as np
            ...
            def calibrateMag_process_one_micrograph(imageFile, apix, res_low, res_high, r_samples, theta_samples):
                import mrcfile
                with mrcfile.open(imageFile) as mrc:
                    images = mrc.data
                if len(images.shape)==2:
                    images = np.expand_dims(images, axis=0)
                fft = fft_resolution_range(images, apix, res_low, res_high, r_samples, theta_samples, return_R_only=False)
                pwr = np.abs(fft)
            ...
            mapping = dict(rlnImagePixelSize="rlnImageName", rlnMicrographPixelSize="rlnMicrographName")
            imageFiles = data[mapping[pixelSize_source]].str.split("@", expand=True).iloc[:,-1].unique()

            from tqdm import tqdm
            from joblib import Parallel, delayed
            results = list(
                tqdm(
                    Parallel(return_as="generator", n_jobs=args.cpu if len(imageFiles)>1 else 1)(
                        delayed(calibrateMag_process_one_micrograph)(imageFile, apix, res_low, res_high, r_samples, theta_samples) for imageFile in imageFiles),
                    unit = "micrograph",
                    total=len(imageFiles),
                    disable=len(imageFiles)<2 or (args.cpu>1 and args.verbose<2) or (args.cpu==1 and args.verbose!=2)
                )
            )
            ...
            pwr_mean = np.sum(pwr_curves * np.expand_dims(n_ptcls, axis=1), axis=0) / n_ptcls.sum()
            ...
            with mrcfile.open(imageFiles[0]) as mrc:
                images = mrc.data
            R = fft_resolution_range(images[0], apix, res_low, res_high, r_samples, theta_samples=180, return_R_only=True)
            res_peak = 1/R[np.argmax(pwr_mean)]
            apix_new = round(apix * target_res / res_peak, 3)   # precision: 0.1%
            if args.verbose>1:
                outputFile = os.path.splitext(args.output_starFile)[0] + f".calibrateMag.{pixelSize_source}={apix}.txt"
                np.savetxt(outputFile, np.hstack((R.reshape((len(R), 1)), pwr_mean.reshape((len(R), 1)))))
                print(f"\tAverage power spectra saved to {outputFile} using the original {pixelSize_source} {apix}")
                import matplotlib.pyplot as plt
            ...
            if apix_new != apix:
                if args.verbose>1:
                    R2 = R * apix/apix_new
                    outputFile2 = os.path.splitext(args.output_starFile)[0] + f".calibrateMag.{pixelSize_source}={apix_new}.txt"
                    np.savetxt(outputFile2, np.hstack((R2.reshape((len(R), 1)), pwr_mean.reshape((len(R), 1)))))
                    print(f"\tAverage power spectra also saved to {outputFile2} using the new, calibrated {pixelSize_source} {apix_new}")
                    plt.plot(R2, pwr_mean, label=f"calibrated pixel size={apix_new}")
                setPixelSize(data, apix_new=apix_new, update_defocus=True)
                if args.verbose>0:
                    print(f"\tCalibrated {pixelSize_source}: {apix_new} ({100*(apix_new-apix)/apix:.1f}% from the original {pixelSize_source} {apix}). {pixelSize_source}, rlnDefocusU, and rlnDefocusV have been updated to use the calibrated {pixelSize_source} {apix_new}")
            else:
                if args.verbose>0:
                    print(f"\tCongratulations! Your original {pixelSize_source} {apix} is accurate without a need to adjust")
            if args.verbose>1:
            ...
            for mgraphName, mgraphParticles in mgraphs:
                ...
                mcount+=1
                if args.verbose:
                    print(("\tMicrograph %d/%d: %d particles from %s are processed and saved to %s" % (
                    mcount, len(mgraphs), pcount, filename, newfilename)))
                data.loc[mgraphParticles.index, "rlnImageName"] = (pd.Series(list(range(1, pcount + 1))).map('{:06d}'.format) + "@" + newfilename).tolist()
            index_d[option_name] += 1
            ...
            param_dict = helicon.parsemodopt2(param)
            value_sigma = param_dict.get("value_sigma", 4.0)  # value_sigma fold of mad above median
            ...
            if gradient_sigma==0:
                ...
                if ny > 2048 and nx > 2048:
                    ...
                    if args.verbose>1:
                        print(f"\tgradient_sigma is set to {gradient_sigma:.1f} Å to remove brightness gradient of the micrographs ({nx}x{ny} pixels)")
                        ...
                        with mrcfile.mmap(outputFile.as_posix()) as mrc:
                            ...
                            if n == len(mgraphParticles):
                                if n>1 or attrSrc in ["rlnImageName"]:
                                    data.loc[mgraphParticles.index, attSrc] = (pd.Series(list(range(1, n + 1))).map('{:06d}'.format) + "@" + outputFile.as_posix()).tolist()
                                else:
                                    data.loc[mgraphParticles.index, attSrc] = outputFile.as_posix()
                                if args.verbose>1:
                                    if attrSrc in ["rlnMicrographName"]:
                                        print(f"\tMicrograph {mi+1}/{len(mgraphs)}: {mgraphName} -> {outputFile.as_posix()} already done. skipped")
                                    else:
                                        print(f"\tMicrograph {mi+1}/{len(mgraphs)}: {n} particles from {mgraphName} -> {outputFile.as_posix()} already done. skipped")
                                continue
                if args.verbose>1:
            ...
            if tasks:
                ...
                from joblib import Parallel, delayed
                results = Parallel(n_jobs=cpu, verbose=max(0, args.verbose-2), prefer="threads")(
                    delayed(maskGold_process_one_micrograph)(t[0], t[1], value_sigma, gradient_sigma, min_area, both_sides, t[2], max(0, args.verbose-2)) for t in tasks)
                for result in results:
                    indices, newImageFile = result
                    data.loc[indices, "rlnImageName"] = (pd.Series(list(range(1, len(indices) + 1))).map('{:06d}'.format) + "@" + newImageFile.as_posix()).tolist()

            data.drop(["tmp_mgraph_name", "tmp_mgraph_pid"], inplace=True, axis=1)
            index_d[option_name] += 1

        elif option_name == "estimateHelicalAngleVariance" and param:
            missing_attrs = [p for p in "rlnImageName rlnHelicalTubeID rlnAngleTilt rlnAnglePsi rlnAngleRot".split() if p not in data]
            assert missing_attrs == [], f"\tERROR: requried parameters {' '.join(missing_attrs)} are not available"
            ...
            from tqdm import tqdm
            for group_particles in tqdm(groups, unit=" filaments", disable=args.verbose<1):
                ...
                psi = np.rad2deg(np.arccos(np.cos(2*np.deg2rad(psi))))   # to make the psi angles independent of polarity
            ...
            if args.verbose>1:
                ...
                for ai, angle in enumerate("Tilt Psi Rot".split()):
                    axes[0, ai].hist(data[f"rlnAngle{angle}"], bins=50, edgecolor='white', linewidth=1)
                ...
                angles = [("Tilt", tilt_sigmas), ("Psi", psi_sigmas), ("Rot", rot_sigmas)]
                for ai, angle in enumerate(angles):
                    ...
                    hbin = axes[3, ai].hexbin(nsegments, angle_sigma, bins='log', gridsize=50, cmap='jet')
                    fig.colorbar(hbin, ax= axes[3, ai], label='# Filaments')
                    axes[1, ai].set(xlabel=f'{angle_str} Sigma (°)', ylabel='# Filaments')
                    axes[2, ai].set(xlabel='Rank (# Filaments)', ylabel=f'{angle_str} Sigma (°)')
                    axes[3, ai].set(xlabel='Filament Length (# Segments)', ylabel=f'{angle_str} Sigma (°)')
                from itertools import combinations
                for pi, pair in enumerate(combinations(angles, 2)):
                    (angle_str_1, angle_sigma_1), (angle_str_2, angle_sigma_2) = pair
                    hbin = axes[pi+1, 3].hexbin(angle_sigma_1, angle_sigma_2, bins='log', gridsize=50, cmap='jet')
                    fig.colorbar(hbin, ax= axes[pi+1, 3], label='# Filaments')
                    axes[pi+1, 3].set(xlabel=f'{angle_str_1} Sigma (°)', ylabel=f'{angle_str_2} Sigma (°)')
                plt.savefig(f"{os.path.splitext(args.output_starFile)[0]}.tilt_psi_rot_sigma.pdf")
                plt.tight_layout()
                plt.show()

        elif option_name == "estimateHelicalTubeLength" and param:
            badParms = [v for v in "rlnImageName rlnHelicalTubeID rlnCoordinateX rlnCoordinateY".split() if v not in data]
            if badParms:
                s = "s" if len(badParms)>1 else ""
                helicon.color_print(f"\tERROR: parameter{s} {' '.join(badParms)} do not exist")
                sys.exit(-1)
            ...
            try:
                ...
                if apix_micrograph<=0:
                    helicon.color_print("\tERROR: neither rlnMicrographPixelSize nor rlnMicrographOriginalPixelSize is available")
                    sys.exit(-1)

            data = reset_inter_segment_distance(data, new_inter_segment_distance=param, apix_micrograph=apix_micrograph, verbose=args.verbose)
            ...
            for v in "rlnMicrographName rlnImageName".split():
                if v in data:
                    ...
                    break
            if not var:
                helicon.color_print(f"\tERROR: rlnMicrographName or rlnImageName must be available")
                sys.exit(-1)
            if "rlnHelicalTubeID" not in data:
                helicon.color_print(f"\tERROR: parameter rlnHelicalTubeID is not available")
                sys.exit(-1)
            ...
            data = data.groupby([var, "rlnHelicalTubeID"], as_index=False, sort=False).first()
            if var == "filename":
            ...
            for ogName, ogData in ogs:
                ...
                mapping = {p:pi+1+og_count for pi, p in enumerate(unique_beam_shift_pos)}
                if args.verbose>10:
                    print(f"{mapping=}")
            ...
            for mgraphName, mgraphParticles in mgraphs:
                tmpStarFile = "%s.%s.star" % (prefix, os.path.basename(os.path.splitext(mgraphName)[0]))
                tmpdata = data.loc[mgraphParticles.index]
                helicon.dataframe2file(tmpdata, tmpStarFile)
                count += 1
                if args.verbose>1:
                    print("\t%d/%d: %d images saved to %s" % (count, len(mgraphs), len(mgraphParticles), tmpStarFile))
            sys.exit()
            ...
            data.drop(tmpCol, inplace=True, axis=1)
            if args.verbose>1:
                print(f"\tThe create time of {len(files):,} {fileAttr} files added to a new column {timeCol}")
    ...
    if args.path != "absolute":
        ...
        if relion_proj_folder:
            for attr in ["rlnImageName", "rlnMicrographName"]:
                if attr not in data: continue
                data[attr] = convert_dataframe_file_path(data, attr, to="relative", relpath_start=relion_proj_folder)

    if args.splitNumSets > 1:
        subsets = [[] for i in range(args.splitNumSets)]
        if args.splitMode in ["micrograph", "helicaltube"]:
            mapping = {"micrograph":"rlnMicrographName", "helicaltube":"rlnHelicalTubeID"}
            var = mapping[args.splitMode]
            if var not in data:
                helicon.color_print(("\tERROR: --splitMode=%s requires \"%s\" in the input star file" % (args.splitMode, var)))
                sys.exit(-1)
            if var == "rlnHelicalTubeID":
                var = ["rlnMicrographName", "rlnHelicalTubeID"]
            mgraphs = data.groupby(var, sort=False)
            mgraphs = sorted(mgraphs, key=lambda x: len(x[1]), reverse=True)  # largest group -> smallest group
            for mgraphName, mgraphParticles in mgraphs:
        ...
        if args.verbose:
            ...
            if filename:
                ...
                if os.path.exists(filename):
                    ...
                    print(f"{len(data):,} images ({nx}x{ny}) saved to {args.output_starFile}. Storage needed: {round(x,1):g} {unit}")
                else:
                    print(f"{len(data):,} images saved to {args.output_starFile}")
            else:
                print(f"{len(data):,} images saved to {args.output_starFile}")

def maskGold_process_one_micrograph(mgraphParticles, outputFile, value_sigma=4, gradient_sigma=50, min_area=200, both_sides=1, msg=None, verbose=0):
    def findGoldMask(data, value_sigma=4, min_area=200, both_sides=1):
        ...
        if both_sides:   # remove negative and positive sides (pixels with large values or small values)
            markers[np.abs(data - median) < (value_sigma-1.) * mad] = 1
            ...
            markers[data < median + (value_sigma-1.) * mad] = 1
            markers[data > median + value_sigma * mad] = 2
    ...
    for row in tqdm(mgraphParticles.itertuples(), unit=unit, disable=verbose!=1):
        ...
        if gradient_sigma>0:
            ...
            data_blurred = skimage.filters.gaussian(data, sigma=gradient_sigma/apix, mode='reflect')
            data -= data_blurred
        ...
        if verbose>1:
            ...
            if len(areas)>1:
                print(f"\t{outputFile.as_posix()} {i+1}/{len(mgraphParticles)}: {np.count_nonzero(goldmask)*apix2:.1f} Å^2 in {len(areas)} regions ({areas[0]:.1f} - {areas[-1]:.1f} Å^2) are masked")
            elif len(areas)==1:
                print(f"\t{outputFile.as_posix()} {i+1}/{len(mgraphParticles)}: {np.count_nonzero(goldmask)*apix2:.1f} Å^2 in 1 region are masked")
            else:
                print(f"\t{outputFile.as_posix()} {i+1}/{len(mgraphParticles)}: nothing to mask")
...
def estimate_inter_segment_distance(data):
    ...
    for _, particles in helices:
        ...
        dists = np.sort(particles["rlnHelicalTrackLengthAngst"].astype(np.float32).values)
...
def reset_inter_segment_distance(data, new_inter_segment_distance, apix_micrograph, current_inter_segment_distance=-1, verbose=0):
    if current_inter_segment_distance>0 and new_inter_segment_distance==current_inter_segment_distance:
        return data
    ...
    for _, particles in tqdm(helices, unit=' helicaltubes', disable=verbose<1):
        if len(particles)<2:
            ...
            continue
        particles_sorted = particles.sort_values(["risd_pid"], ascending=True).reset_index(drop=True)
        x = particles_sorted.loc[:, "rlnCoordinateX"].astype(float).values # pixel
        y = particles_sorted.loc[:, "rlnCoordinateY"].astype(float).values
        pos, xy_fit = helicon.line_fit_projection(x, y, w=None, ref_i=0, return_xy_fit=True)
        n0 = len(pos)
        ...
        if "rlnHelicalTrackLengthAngst" in df_tmp:
            df_tmp.loc[:, "rlnHelicalTrackLengthAngst"] = (pos_new - pos_new[0]) * apix_micrograph
        data2.append(df_tmp)     
...
def estimate_helicalTube_length(data, inter_segment_distance=-1, verbose=0):
    ...
    for _, particles in tqdm(helices, unit=' helicaltubes', disable=verbose<1):
        if "rlnHelicalTrackLengthAngst" in particles:
            data.loc[particles.index, "rlnHelicalTubeLength"] = round(particles["rlnHelicalTrackLengthAngst"].max(), 1)
        else:
    ...
    for source in sources:
        ...
        for attr in attrs:
            if attr in source:
                ...
                if return_pixelSize_source: return apix, attr
                return apix
    return None
...
def readCtfparmFile(filename):  # EMAN1 ctfparm.txt format file
    ...
    for l in fp.readlines():
        ...
        params = rest.split(",")
        if len(params) == 14:
            defocus, dfdiff, dfang, bfactor, amplitude, ampcont, noise1, noise2, noise3, noise4, voltage, cs, apix = list(map(
                float, params[:13]))
        elif len(params) == 12:
            defocus, bfactor, amplitude, ampcont, noise1, noise2, noise3, noise4, voltage, cs, apix = list(map(float,
                                                                                                          params[:11]))
            dfdiff = 0
            dfang = 0
        else:
            helicon.color_print("\tERROR: wrong format for line in %s\n%s\n" % (filename, l))
            sys.exit(1)
        d = {}
...
def add_args(parser):
    ...
    parser.add_argument("--csparcPassthroughFiles", metavar="<filename>", type=str, nargs="+",
                        help="input cryosparc v2 passthrough file(s)", default=[])
    parser.add_argument("--first", type=int, metavar="<n>", help="first image to process. default to 0",
                        default=0)
    parser.add_argument("--last", type=int, metavar="<n>",
                        help="last image to process. default to the last image in the file", default=-1)
    parser.add_argument("--subset", metavar="<n>", type=int, help="which subset to keep. default to 0", default=0)
    parser.add_argument("--sets", metavar="<n>", type=int, help="number of subsets to split into. default to 1",
                        default=1)
    parser.add_argument("--splitNumSets", metavar="<n>", type=int, help="number of subsets to split into. default to 1",
                        default=1)
    splitMode = ["evenodd", "random", "micrograph", "helicaltube"]
    parser.add_argument("--splitMode", metavar="<%s>" % ("|".join(splitMode)), type=str, choices=splitMode,
                        help="how to split image set", default="evenodd")
    parser.add_argument("--randomSample", metavar="<n>", type=int,
                        help="take random n images subset. disabled by default", default=0)
    parser.add_argument("--recoverFullFilaments", metavar="minFraction=<0.5>[:forcePickJob=<0|1>][:fullStarFile=<filename>]", type=str,
                        help="recover the whole filaments if current filament has >=minFraction of the segments in the whole filament", default="")
    parser.add_argument("--select", type=str, metavar=("<var>", "<val1<,val2>...>"), nargs=2,
                        help="select images with exact matching of the specified variable value(s). disabled by default",
                        default=[])
    parser.add_argument("--selectValueRange", type=str,
                        metavar=("<var>", "<valmin>", "<valmax>"), nargs=3,
                        help="select images with the variable value in the specified range. disabled by default",
                        default=[])
    parser.add_argument("--selectRatioRange", type=str,
                        metavar=("<var>", "<ratio min>", "<ratio max>"), nargs=3,
                        help="select images with the variable value in the specified ratio range. disabled by default",
                        default=[])
    parser.add_argument("--selectByParticleLocation", type=str, metavar="starFile:maxDist=<pixel>", action="append", help="select particles that are at the same locations in the micrograph (example: x.star:maxDist=10). disabled by default", default=[])
    parser.add_argument("--selectFile", type=str, metavar="starFile[:col1=<name>:col2=<name>:pattern=<str>]", action="append", help="select images in the specified file (example: x.star:col1=rlnImageName:col2=rlnImageOriginalName:pattern=\".*(16jul.*-a).*\"). disabled by default", default=[])
    parser.add_argument("--excludeFile", type=str, metavar="starFile[:col1=<name>:col2=<name>:pattern=<str>]", action="append", help="exclude images in the specified file (example: x.star:col1=rlnImageName:col2=rlnImageOriginalName:pattern=\".*(16jul.*-a).*\"). disabled by default", default=[])
    parser.add_argument("--sortby", type=str, action="append",
                        metavar="<parameter>", nargs='+',
                        help="sort (small to large) by the specified parameter(s). disabled by default", default=[])
    parser.add_argument("--rsortby", type=str, action="append",
                        metavar="<parameter>", nargs='+',
                        help="reverse sort (large to small) by the specified parameter(s). disabled by default", default=[])
    parser.add_argument("--minDuplicates", metavar="<n>", type=int,
                        help="only keep images >=n duplicates. disabled by default", default=0)
    parser.add_argument("--removeDuplicates", metavar="<var>", nargs="+", type=str,
                        help="remove images with duplicate parameters. disabled by default", default="")
    parser.add_argument("--normEulerDist", type=float, metavar=("<angle bin size>", "<nkeep>"), nargs=2,
                        help="reduce Euler (view) preference by removing the worst particles in over-populated angular bins",
                        default=[])
    parser.add_argument("--setCTF", metavar="<filename>", type=str,
                        help="set ctf parameters stored in this file (EMAN1 ctfparm.txt) to the output star file",
                        default="")
    parser.add_argument("--copyCtf", metavar="<starfile>", type=str, help="Star file to copy CTF parameters from. Should be a CTF refinement output file.")
    parser.add_argument("--setParm", metavar=("<var> <val>"), type=str, nargs="+",
                        help="set parameter var val pair for each image", action="append", default=[])
    parser.add_argument("--setBeamTiltClass", metavar="<0|1>", type=int,
                        help="set rlnBeamTiltClass column, one group per micrograph", default=0)
    parser.add_argument("--keepParm", metavar="<var>", type=str, nargs="+",
                        help="keep parameter var for each image, remove other parameters", action='append', default=[])
    parser.add_argument("--delParm", metavar="<var>", type=str, nargs="+", action='append',
                        help="remove parameter var for each image", default=[])
    parser.add_argument("--copyParm", metavar="<starfile< var ~var ...>>", type=str, nargs="+",
                        help="copy the specified parameters or all parameters if no var is specified. ~var will skip copying var", default=[])
    parser.add_argument("--addParm", metavar="<var> <val>", type=str, nargs=2, help="modify parameter: var+=val", action="append", default=[])
    parser.add_argument("--multParm", metavar="<var> <val>", type=str, nargs=2, help="modify parameter: var*=val", action="append", default=[])
    parser.add_argument("--duplicateParm", metavar="<from> <to>", type=str, nargs=2, help="duplicate parameter", action="append", default=[])
    parser.add_argument("--renameParm", metavar="<old> <new>", type=str, nargs=2, help="rename parameter", action="append", default=[])
    parser.add_argument("--psiPrior180", metavar="<0|1>", type=int, help="duplicate data by adding 180 degrees to rlnAnglePsiPrior", default=0)
    parser.add_argument("--replaceStr", metavar=("<var>", "<original str>", "<new str>"), type=str, nargs=3,
                        help="replace substr in the variable with new str", action='append', default=[])
    parser.add_argument("--replaceImageName", metavar="<new mrcs file>", type=str,
                        help="replace rlnImageName column by the provided mrcs file that has the same number of particles", default="")
    parser.add_argument("--apix", type=float, metavar="<A/pixel>", help="set mag to have this sampling",
                        default=0)
    parser.add_argument("--path", metavar='<absolute|relative|real|shortest|current>', type=str,
                        choices=["absolute", "relative", "real", "shortest", "current"],
                        help="which type of file path is used for the images. default to current", default="current")
    ...
    parser.add_argument("--minStack", type=int, metavar="<0|1>",
                        help="generate a new set of mrcs files including only the subset of particles in this stack. default to 0", default=0)
    parser.add_argument("--fullStack", type=int, metavar="<0|1>",
                        help="generate a star stack including all particles in the referenced image files. default to 0", default=0)
    parser.add_argument("--createStack", dest="createStack", type=str, metavar="output.mrcs:rescale2size=<n>:float16=<0|1>:force=<0|1>", help="create a new mrcs file to store all particles", default=None)
    choices = "graphene graphen_oxide go gold ice".split()
    parser.add_argument("--calibratePixelSize", metavar="<%s>" % ("|".join(choices)), type=str, choices=choices, help="calibrate mag based on known diffraction peak resolution of standards such as graphene/graphene oxide at 2.13Å or gold at 2.355Å", default=None)
    parser.add_argument("--process", metavar="processor_name:param1=value1:param2=value2", type=str, nargs="+", action="append",
                        help="apply a processor named 'processorname' with all its parameters/values.")
    parser.add_argument("--maskGold", metavar="value_sigma=<n>:gradient_sigma=<Å>:min_area=<Å^2>:both_sides=<0|1>:outdir=<str>:force=<0|1>:cpu=<n>", type=str, action="append", help="mask out electron dense (gold, ferritin, ice) pixels in images. disabled by default", default=None)
    parser.add_argument("--estimateHelicalAngleVariance", metavar="<0|1>", type=int, help="estimate the variance of the tilt, psi, rot angles of segments in the same helical tube/filament", default=0)
    parser.add_argument("--estimateHelicalTubeLength", metavar="<0|1>", type=int, help="estimate the length of each helical filament/tube", default=0)
    parser.add_argument("--resetInterSegmentDistance", metavar="<Å>", type=float, help="reset inter-segment distance by adding/removing 'particles' with updated 'rlnCoordinateX rlnCoordinateY rlnHelicalTrackLengthAngst' parameters. Warning: the output star file is meaningful only for particle extraction and it should NOT be used for 2d/3d classification or 3d refinement", default=0)
    parser.add_argument("--keepOneParticlePerHelicalTube", metavar="<0|1>", type=int, help="keep only one segment of each helical filament/tube", default=0)
    parser.add_argument("--keepOneParticlePerMicrograph", metavar="<0|1>", type=int, help="keep only one segment of each micrograph", default=0)
    parser.add_argument("--showTime", metavar="<attr>", type=str, help="include file create time of the attr in the output star file. disabled by default", default=None)
    parser.add_argument("--tag", metavar="<str>", type=str, help="add this tag to new binary image files", default="")
    parser.add_argument("--folder", metavar="<dirname>", type=str, nargs="*", help="Search these folders if the images cannot be found. default: \"\"", default=[])
    ...
    parser.add_argument("--force", type=int, metavar="<0|1>", help="force overwrite output images. default to 0", default=0)
    parser.add_argument("--ppid", metavar="<n>", type=int, help="Set the PID of the parent process, used for cross platform PPID", default=-1)
...
def check_args(args, parser):
    ...
    if os.path.exists(args.output_starFile) and not (args.force==1 or args.splitNumSets>1):
        helicon.color_print("\tERROR: the output file (%s) exists. Use --force=1 to overwrite it" % (args.output_starFile))
        sys.exit(-1)

    if args.setCTF and not os.path.exists(args.setCTF):
        helicon.color_print("ERROR: option \"--setCTF %s\" specifies of a nonexistent file" % (args.setCTF))
        sys.exit(1)
