...
from scipy.stats import vonmises, pearsonr, norm
...
def plot_one(ax, j):
    """
    Replace this with your own drawing logic for plot #i.
    Keep it fast & side-effect free; draw only into `ax`.
    """
...
def jpgs_to_pdf(jpg_paths, pdf_path, dpi=200):
    """
    Combine a list of JPGs into a single multi-page PDF.

    jpg_paths : list[str or Path]  # ordered list of images
    pdf_path  : str or Path        # output PDF path
    dpi       : int                # nominal output DPI for PDF metadata
    """
    ...
    first.save(pdf_path, "PDF", resolution=float(dpi), save_all=True, append_images=rest)
    return str(pdf_path)



def wrap_sym(z, P):
    """Wrap to symmetric interval [-P/2, P/2)."""
    return (z + P/2) % P - P/2

def unwrap_sequence(y, P):
    """
    Phase-unwarp a 1D sequence y measured modulo P:
    adjust by multiples of P so consecutive diffs lie in [-P/2, P/2).
    """
...
def solve_b_given_m_wrapped(x, o, P, m):
    """
    Given slope m and period P (e.g., 2π or 360), solve L2-optimal b.
    Steps:
      1) r0 = o - m*x   (raw residuals, still modulo P)
      2) r  = unwrap_sequence(r0, P)
      3) b  = mean(r)
      4) Score with shortest-arc residuals d = wrap_sym(o - (m*x + b), P)
    Returns b, SSE.
    """
...
def fit_line_wrapped_by_m_grid(x, o, P, m_min, m_max, num_m=501):
    """
    Grid m in [m_min, m_max]; for each m, compute b via the wrapped approach above.
    Return best (m, b, SSE) and the grids for inspection.
    """
    ...
    return float(m_grid[i]), float(b_grid[i]), float(S_grid[i]), (m_grid, b_grid, S_grid)




# --- 1) Plain (non-wrapped) data ---
def plot_linear_fit_simple(x, o, m_best, b_best,P=2.*np.pi):
    x = np.asarray(x, float)
    o = np.asarray(o, float)
...
def get_angular_errors(x, o, m_best, b_best,Period):
    yy = m_best * x + b_best ; # This is the fit on the resampled data
...
def l1_core_average(x, k=50):
    """
    Returns:
      m_best: the requested 'average'
      l1_sum: minimal sum of absolute deviations over the closest k points
      subset: the k points (contiguous in sorted order) that determine m_best
    """
...
def HelicalSegmentConsistency(
    data: pd.DataFrame,
    *,
    convert_path_fn=None,      # ignored on purpose (we won’t hit the filesystem)
    verbose: int = 0,
    input_star_path: str  | None = None,
    output_star_path: str | None = None,
    param: str | None = None,):
    #def HelicalSegmentConsistency(data, *, convert_path_fn=None, verbose=0, output_star_path=None, param=None):
    ...
    parts = data.copy()
    print(parts.columns)
    print('Total # of columns ='  + str(len(parts.columns)))
    ...
    if 0: OutputFigsDir = 'Figs'+stamp
    if not os.path.isdir(OutputFigsDir):
        os.mkdir(OutputFigsDir)

    
    print(OutputFigsDir)
    OUTPDFDIR = Path(OutputFigsDir+'/Stuff/'); #  This is the subdirectory  where the jpg and PDFs get stored
    pdf_path =  'batch_'+ StarFileName[:-5]+'.pdf'; # This is the filename within Stuff
    ...
    print("PSU date and time:", now_local.strftime("%Y-%m-%d %H:%M:%S %Z"))

    stats = dict()
    """
        nsegments=nsegments,
        tilt_means=tilt_means,
        tilt_sigmas=tilt_sigmas,
        psi_sigmas=psi_sigmas,
        rot_sigmas=rot_sigmas,
        options=opts,)
    """


    #       Section 1    create some auxiliary columns
    print('Section 1    create some auxiliary columns')
    ...
    micro_to_id = {name: i for i, name in enumerate(parts["rlnMicrographFromImageName"].unique())}
    ...
    print("min/max rlnPartNum:", int(parts["rlnPartNum"].min()), int(parts["rlnPartNum"].max()))
    print("unique micrographs:", len(MicrographList))
    
    
    # Collect unique micrograph list
    MicrographList = list(micro_to_id.keys())
    print("Number of unique micrographs:", len(MicrographList))   # expect 3514
    if 0:
    ...
    print('Section 2  create some categorical index')

    
    parts["rlnHelicalTubeAndMicID"] = (parts.groupby(["rlnMicUniqId", "rlnHelicalTubeID"])
                                                      .ngroup()     )
    ...
    # check result
    if 0:
        print(parts[["rlnMicUniqId", "rlnHelicalTubeID", "rlnHelicalTubeAndMicID"]].head(5))
    NumberUniqueHelices =  parts["rlnHelicalTubeAndMicID"].nunique()
    print("Number of unique helices:", NumberUniqueHelices)
    if 0:
    ...
    now_utc = datetime.datetime.now(datetime.timezone.utc)
    now_local = now_utc.astimezone(local_tz)
    print("Section 2.5 PSU date and time:", now_local.strftime("%Y-%m-%d %H:%M:%S %Z"))
    ...
    print('HelixMicID,NumSegments, AveAngles')
    ...
    for HelixMicID in range(NumberUniqueHelices):
        ...
        if NumSegments <NumSegmentsMin: continue
    ...
    print(f"NumSegmentsAllArray          = {np.sum(NumSegmentsAllArray)}")
    print(f"NumSegmentsManySegmentsArray = {np.sum(NumSegmentsManySegmentsArray)}")
    print(f"Total HelixMicIDsManySegments = {len(HelixMicIDsManySegments)}")
    ...
    print('AllRotArrayBreadth = ' +str(AllRotArrayBreadth))
    
    SymPre = 360/AllRotArrayBreadth
    SymGuess  = int(SymPre+.3)
    
    print("Guess for Symmetry = " +str(SymGuess))
    
    if SymGuess==1: SymGuess=2
    Period = 360/SymGuess
    ...
    print('Section 3: Get listing of all the slopes.  And make global plots')
    ...
    for HelixMicID in range(NumberUniqueHelices):
        ...
        m_best, b_best, Smin, _ = fit_line_wrapped_by_m_grid(idx, angles_deg, Period, m_min=-1.1, m_max=1.1, num_m=num_m )
    ...
    HelixMicIDsManySegmentsMin    = HelixMicIDsManySegments[SminManySegmentsInd] ; #  27
    mManySegmentsMin              = mManySegmentsArray[SminManySegmentsInd]; # 0.245
    #mBest = np.abs(mManySegmentsMin)
    
    
    
    print(' StarFile,  HelixMicIDsManySegmentsMin, SminManySegmentsMin, SminManySegmentsInd, mManySegmentsMin, Period')
    print( StarFileName,  HelixMicIDsManySegmentsMin, SminManySegmentsMin, SminManySegmentsInd, mManySegmentsMin, Period)
    ...
    sub = parts[(parts["rlnHelicalTubeAndMicID"] == HelicalTubeAndMicID) ]  # 27 for Fahim Data
...
###################################################3
    ...
    print('Number Of long Helices for '+StarFileName +' = ' + str(Count) )
    
    print('Total Segments At Start = ' + str( np.sum(NumSegmentsAllArray)))
    print('Total Segments In Long Helices = ' +str( np.sum(NumSegmentsManySegmentsArray)))
    ...
    print('Total # long Helices (num segments >25)=' +str(NumLongHelices)) 
    ...
    print('len(mNP),sum(mask)')
    print(len(mNP),sum(mask))
    
    print('Total # long Helices (num segments >25) and Smin<10000=' +str(sum(mask))) 
    plt.figure(figsize=(12,3))
...
#      Section 4.3 Initialize two new columns in dataframe (which becomes output star)
    ...
    print('HelixMicIDsBest = '+str(HelixMicIDsBest))
    print('mBest = '+str(mBest))
    print('Period = '+str(Period))
    
    print('HelixMicIDsManySegmentsMin = '+str(HelixMicIDsManySegmentsMin)) 
    ...
    NumSegments = subid.shape[0]
    print('Globally Best has NumSegments='+str(NumSegments))
    ...
    mFinal, bFinal, SSEfinal  = fit_line_wrapped_by_m_known(idx, angles_deg, Period, mBest )
    xx,yy,_ = plot_linear_fit_simple(idx, angles_deg, mFinal, bFinal, Period)
    
    #SminManySegmentsArray.append(Smin)
    #mManySegmentsArray.append(m_best)
    
    print('HelixMicID,NumSegments, m_best, Smin, b_best')
    print(HelixMicID,NumSegments, f"{mFinal:.3g}", f"{Smin:.3g}", f"{bFinal:.3g}")
    ...
    #plt.xlabel("Segment index along filament")
    plt.xlabel("Angstroms along filament")
    plt.title("AngleRot for HelixMicID="+str(HelixMicID)+"\n  Smin="+  format(SSEfinal, ".2f") +" mFinal="+  format(mFinal, ".3f") )
    
    
    plt.savefig("AnotherGoodHelixNP.jpg")
    
    print( 'mBest = '+str(abs(mBest)))
    ...
    for j, HelixMicID in enumerate(HelixMicIDsManySegments):
        ...
        plt.title("AngleRot for "+str(HelixMicID)+"\n  Smin="+  format(Smin, ".2f") +"\n  m_best="+  format(m_best, ".2f") )
    ...
    print(' This is the number of long helices = ' +str(len(m1NP)))
    ...
    print('Section 6. Order Smin and bind all outputs for long chains all into a pdf')
    ...
    def plot_one_unwrapped(ax, j, m_best):
        ...
        #m_best, b_best, Smin, _ = fit_line_wrapped_by_m_grid(idx, angles_deg, P, m_min=-1.2, m_max=1.2, num_m=num_m )
        #m_signed, b_best, Smin = fit_line_wrapped_by_m_known(idx, angles_deg, P, m_best )
        
        if 0:
            mFinal, bFinal, SSEfinal  = fit_line_wrapped_by_m_known(idx, angles_deg, Period, m_best )
            #print(f"m_best={m_best:.4f}, b_best={b_best:.4f}, Smin={Smin:.4f}")
            xx,yy,_ = plot_linear_fit_simple(idx, angles_deg, mFinal, bFinal,Period)
        
            AngErrors, AbsAngErrors = get_angular_errors(idx, angles_deg, mFinal, bFinal,Period)
    
        else:
            mFinal, bFinal, SSEfinal  = fit_line_wrapped_by_m_known(idxSortedOnTrack, angles_sorted_on_track, Period, m_best )
            #print(f"m_best={m_best:.4f}, b_best={b_best:.4f}, Smin={Smin:.4f}")
            xx,yy,_ = plot_linear_fit_simple(idxSortedOnTrack, angles_sorted_on_track, mFinal, bFinal,Period)
        
            AngErrors, AbsAngErrors = get_angular_errors(idxSortedOnTrack, angles_sorted_on_track, mFinal, bFinal,Period)
    
        # !!!!!!!!!!!
        
        parts.loc[parts["rlnHelicalTubeAndMicID"] == HelixMicID, "rlnHelicalTubeAndMicIDGood"] = np.mean(AbsAngErrors);
        ...
        ax.set_title("HelixMicID="+str(HelixMicID)+"\n  Smin="+  format(SSEfinal/100, ".2f") +";  m ="+  format(mFinal, ".2f"))
    ...
    print('Section 7. More plots toward decision functions')
    ...
    if 0:
        ...
        mFinal, bFinal, SSEfinal  = fit_line_wrapped_by_m_known(idxSortedOnTrack, angles_sorted_on_track, Period, mBest )
        xx,yy,_ = plot_linear_fit_simple(idxSortedOnTrack, angles_sorted_on_track, mFinal, bFinal,Period)
        AngErrors, AbsAngErrors = get_angular_errors(idxSortedOnTrack, angles_sorted_on_track, mFinal, bFinal,Period)
    ...
    fig.suptitle('Best fit '+StarFileNameKey+' \n Worst error here '+ f"≈ {BiggestErrorOnBestCurve:.1f}"+' degrees', fontsize=12)
    ...
    plt.tight_layout()
    #plt.show()
    print('BiggestErrorOnBestCurve =  ',   f"x ≈ {BiggestErrorOnBestCurve:.1f}" + "Å" )
    ...
    def G2_model(x, APeak, sigmaPeak, ATail, sigmaTail):
        x = np.asarray(x)
        return APeak * np.exp(-0.5 * (x / sigmaPeak)**2) + ATail * np.exp(-0.5 * (x / sigmaTail)**2)
    ...
    if 0:
        popt, pcov = curve_fit( G_model, x_fit, y_fit,  p0=[A0, sigma0],  sigma=y_err, absolute_sigma=False    )
    else:
        popt, pcov = curve_fit(G2_model, x_fit, y_fit,  p0=[A0, sigma0,ATail, sigmaTail],  sigma=y_err, absolute_sigma=False    )
    
    APeak_fit, sigmaPeak_fit ,ATail_fit, sigmaTail_fit= popt
    
    
    if 0: 
        print("A_fit   =", A_fit); print("sigma_fit =", sigma_fit)
    else:
        print("APeak_fit   =", APeak_fit); print("sigmaPeak_fit =", sigmaPeak_fit)
        print("ATail_fit   =", ATail_fit); print("sigmaTail_fit =", sigmaTail_fit)
    ...
    if 0: yy = G_model(xx, A_fit, sigma_fit)
    else: yy = G2_model(xx, APeak_fit, sigmaPeak_fit, ATail_fit, sigmaTail_fit)
    ...
    now_utc = datetime.datetime.now(datetime.timezone.utc)
    now_local = now_utc.astimezone(local_tz)
    print("Section 7 PSU date and time:", now_local.strftime("%Y-%m-%d %H:%M:%S %Z"))
...
def _read_star(path: str) -> pd.DataFrame:
    """
    Read a RELION-style STAR file into a DataFrame.
    Prefer the 'starfile' library if available.
    Assumes a single particles table (common for run_data.star).
    """
    if starfile is None:
        raise RuntimeError(
...
def _write_star(df: pd.DataFrame, path: str, like: str | None = None):
    """
    Write a DataFrame back to .star. If 'like' is provided and readable,
    keep a two-table structure (data_optics + data_particles) using 'like'
    to carry over the optics table; otherwise write a single table.
    """
    if starfile is None:
        raise RuntimeError(
...
def _parse_args(argv=None):
    ...
    p.add_argument("input_star", help="Input STAR (e.g., Refine3D/.../run_data.star)")
    p.add_argument("output_star", help="Output STAR to write")
    p.add_argument("--verbose", type=int, default=1)
    p.add_argument("--param", type=str, default=None,
                   help="Optional JSON or k=v pairs for extra options")
    p.add_argument("--force", type=int, default=0, help="Overwrite output if exists (0/1)")
    return p.parse_args(argv)

def _main(argv=None):
    args = _parse_args(argv)
    if os.path.exists(args.output_star) and not args.force:
        sys.exit(f"Refusing to overwrite existing file: {args.output_star} (use --force 1)")
    ...
    """
    if args.verbose:
        print(f"[HSC] Wrote {args.output_star} | rows={len(df_out)} | "
              f"mean Tilt σ={np.mean(stats['tilt_sigmas']):.2f}°")
    """
