...
def get_relion_project_folder(starFile):
    ...
    if filename_abs.find("/job")==-1: return None
    parts = filename_abs.split("/")
    for pi, p in enumerate(parts):
        if p.startswith("job"): break
    job_folder = '/'.join(parts[:pi+1])
    if not os.path.exists(os.path.join(job_folder, "default_pipeline.star")): return None
    pi = max(0, pi-1)
...
def eman_euler2quaternion(az, alt, phi):
    ...
    alpha_beta_gamma = np.vstack((np.deg2rad(az-90), np.deg2rad(alt), np.deg2rad(phi+90))).T  # z,y,z convention
...
def relion_euler2quaternion(rot, tilt, psi):
    import quaternionic as qtn
    alpha_beta_gamma = np.vstack((np.deg2rad(rot), np.deg2rad(tilt), np.deg2rad(psi))).T  # z,y,z convention
    q = qtn.array.from_euler_angles(alpha_beta_gamma, beta=None, gamma=None).normalized
    q = np.array(q) # qtn.array cannot be pickled for joblib Parallel processing
    return q
...
def quaternion2euler(q, euler_convention="relion"):
    ...
    rot, tilt, psi = alpha_beta_gamma[:, 0], alpha_beta_gamma[:, 1], alpha_beta_gamma[:, 2]
    if len(q.shape)==1:
...
def average_relion_eulers(rot, tilt, psi, weights = None, return_quaternion=False):
    ...
    if weights: assert(len(weights) == len(rot))
...
try:
  ...
  color_print(f"WARNING: failed to load numba. The program will run correctly but will be much slower. Run 'pip install numba' to install numba and speed up the program")
  def jit(*args, **kwargs):
    return lambda f: f
...
def images2dataframe(inputFiles, csparc_passthrough_files=[], alternative_folders=[], ignore_bad_particle_path=0, ignore_bad_micrograph_path=1, warn_missing_ctf=1, target_convention=None):
    if isinstance(inputFiles, str):
        data = image2dataframe(inputFiles, csparc_passthrough_files, alternative_folders, ignore_bad_particle_path, ignore_bad_micrograph_path, warn_missing_ctf)
        if target_convention: data = dataframe_convert(data, target=target_convention)
        return data
...
def image2dataframe(inputFile, csparc_passthrough_files=[], alternative_folders=[], ignore_bad_particle_path=0, ignore_bad_micrograph_path=1, warn_missing_ctf=1):
    ...
    if inputFile.endswith(".star"):    # relion
        p = star2dataframe(inputFile, alternative_folders, ignore_bad_particle_path, ignore_bad_micrograph_path)
    elif inputFile.endswith(".csv"):    # cryosparc v0.x
        p = csv2dataframe(inputFile, alternative_folders, ignore_bad_particle_path, ignore_bad_micrograph_path)
    elif inputFile.endswith(".cs"):  # cryosparc v2.x
        p = cs2dataframe(inputFile, csparc_passthrough_files, alternative_folders, ignore_bad_particle_path, ignore_bad_micrograph_path, warn_missing_ctf)
    elif inputFile.endswith(".db"):  # cisTEM
        # will directly read and convert Relion convention
        p = cistem2dataframe(inputFile, alternative_folders, ignore_bad_particle_path, ignore_bad_micrograph_path)
    else:
        import collections
        data = []
...
def dataframe2file(data, outputFile):
    if len(data)<1:
        color_print(f"WARNING: dataframe2file(data, outputFile={outputFile}): data is empty, nothing to save")
        sys.exit(-1)
    if outputFile.endswith(".oldformat.star"):
        ...
        color_print("ERROR: dataframe2file(data, outputFile=%s) is called with a unsupported file format. Only .star and .cs formats are supported" % (outputFile))
        sys.exit(-1)
...
def dataframe_guess_data_type(data):
    intVars = set("pid ppid frame set class sym helicaltube helicalclass vppid vpppos rlnRandomSubset rlnClassNumber classID rlnHelicalTubeID rlnBeamTiltClass rlnClass3DNumber rlnOpticsGroup rlnImageSize rlnImageDimensionality alignments.model-best.k".split())
    floatVars = set("voltage cs ampcont defocus dfdiff dfang btamp btang vps scale asamp asang score".split())
    floatVars.update(set(['rlnAmplitudeContrast', 'rlnAnglePsi', 'rlnAngleRot', 'rlnAngleTilt', 'rlnCoordinateX', 'rlnCoordinateY', 'rlnDefocusAngle', 'rlnDefocusU', 'rlnDefocusV', 'rlnDetectorPixelSize', 'rlnImagePixelSize', 'rlnLogLikeliContribution', 'rlnMagnification', 'rlnOriginX', 'rlnOriginXAngst', 'rlnOriginY', 'rlnOriginYAngst','rlnPhaseShift', 'rlnSphericalAberration', 'rlnVoltage', 'rlnCtfMaxResolution']))
    floatVars.update(set(['ctf/ctf_fit_to_A']))
    strVars = set("emdid filename micrograph movie rlnImageName rlnMicrographName rlnMicrographMovieName".split())
    bytesVars = set("blob/path micrograph_blob/path location/micrograph_path ctf/path mscope_params/defect_path".split())
    known_type_dict = {}
    unknown_type_cols = []
    bytes2str_cols = []
    for col in data:
        ...
        elif col in floatVars or col.lower().endswith("change") or col.lower().endswith("sigma") or col.lower().endswith("score"):
    ...
    if optics is not None:
        ...
        for gn, g in data.groupby("rlnOpticsGroup", sort=False):
            if gn not in og_names:
                color_print(f"ERROR: optic group {gn} not available ({sorted(og_names)})")
                sys.exit(-1)
            ptcl_indices = g.index
            og_index = optics["rlnOpticsGroup"] == gn
            if "rlnAmplitudeContrast" in optics:
                data.loc[ptcl_indices, "rlnAmplitudeContrast"] = optics.loc[og_index, "rlnAmplitudeContrast"]
            if "rlnImagePixelSize" in optics:
                data.loc[ptcl_indices, "rlnImagePixelSize"] = optics.loc[og_index, "rlnImagePixelSize"]
            if "rlnSphericalAberration" in optics:
                data.loc[ptcl_indices, "rlnSphericalAberration"] = optics.loc[og_index, "rlnSphericalAberration"]
            if "rlnVoltage" in optics:
                data.loc[ptcl_indices, "rlnVoltage"] = optics.loc[og_index, "rlnVoltage"]
...
def star_build_opticsgroup(data):
    ...
    def missing_required_opticsgroup_parameters(data):
        requiredVars = "rlnVoltage rlnSphericalAberration rlnMicrographOriginalPixelSize".split()
    ...
    if not missingVars: return
    
    vars = [v for v in Relion_OpticsGroup_Parameters if v in data and v != "rlnOpticsGroup"]
    if vars:
        ogp_list = []
        for gi, g in enumerate(data.groupby(vars if len(vars)>1 else vars[0], sort=False)):
            gn, gdata = g
            d = {}
    ...
    try:
        ...
        if "rlnImageSize" in missingVars and "rlnImageSize" not in optics:
            ...
            elif "rlnMicrographName" in data: var = "rlnMicrographName"
            if var:
                ...
                if os.path.exists(imageFileName):
                    ...
                    color_print(f"WARNING: failed to obtain rlnImageSize, rlnImageDimensionality from non-existing file{imageFileName}. You should manually add both parameters to the optics group of the star file")
        ...
        '''
        if "rlnMicrographOriginalPixelSize" in missingVars and "rlnMicrographOriginalPixelSize" not in optics:
            if "rlnMicrographPixelSize" in optics:
                optics.loc[:, "rlnMicrographOriginalPixelSize"] = optics.loc[:, "rlnMicrographPixelSize"]
                color_print(f"WARNING: 'rlnMicrographOriginalPixelSize' is copied from 'rlnMicrographPixelSize'. Please manually edit it if it is incorrect")
            elif "rlnImagePixelSize" in optics:
                optics.loc[:, "rlnMicrographOriginalPixelSize"] = optics.loc[:, "rlnImagePixelSize"]
                color_print(f"WARNING: 'rlnMicrographOriginalPixelSize' is copied from 'rlnImagePixelSize'. Please manually edit it if it is incorrect")
    except:
        pass
    missingVars = missing_required_opticsgroup_parameters(data)
    if missingVars: 
        varval = ' '.join([f'{v} <val>' for v in missingVars])
        color_print(f"WARNING: required OpticsGroup parameters {' '.join(missingVars)} are missing. Use \n\timages2star.py <input.star> <output.star> --setParm {varval}\nto add these parameters")

# https://github.com/dzyla/Follow_Relion_gracefully/blob/main/follow_relion_gracefully_lib.py#L352
def star2dataframe(starFile, alternative_folders=[], ignore_bad_particle_path=0, ignore_bad_micrograph_path=1):
    ...
    if nans.sum()>0:
        color_print("WARNING: %s: %d/%d particle rows are corrupted and thus ignored" % (starFile, nans.sum(), len(data)))
        ...
        if not set(suffix).issubset(["mrcs", "mrc", "tnf", "spi", "img", "hed"]):
            ...
            for mi, mgraph in enumerate(mgraphs):
                ...
                if suffix not in [".mrcs", ".mrc", ".tnf", ".spi", ".img", ".hed"]:
                    color_print(f"WARNING: RELION does not support image format: {mgraphName}")
...
def cs2dataframe(csFile, passthrough_files=[], alternative_folders=[], ignore_bad_particle_path=0, ignore_bad_micrograph_path=1, warn_missing_ctf=1):
    ...
    if passthrough_files:
        ...
        if ptfs: passthrough_files_final.append(ptfs[0])
    ...
    if "blob/path" not in data and "micrograph_blob/path" not in data:
        ...
        sys.exit(-1)
    if warn_missing_ctf and "ctf/accel_kv" not in data and csFile.find("templates_selected.cs")==-1:
        color_print("WARNING: CTF info not found. You should also provide the passthrough file that has CTF info")
    if "ctf/type" in data: data = data.drop("ctf/type", axis=1)

    data = dataframe_guess_data_type(data)
    nans = data.isnull().any(axis=1)
    if nans.sum()>0:
        color_print("WARNING: %s: %d/%d particle rows are corrupted and thus ignored" % (csFile, nans.sum(), len(data)))
        color_print("    Corrupted particle indices:\n%s" % (nans.to_numpy().nonzero()[0]))
...
def cistem2dataframe(dbFile, alternative_folders=[], ignore_bad_particle_path=0, ignore_bad_micrograph_path=1):
    import sqlalchemy
    if dbFile.find('@') == -1:
        iter = -1
    else:
    ...
    refinement_list = pd.read_sql_table("REFINEMENT_LIST", db).set_index("REFINEMENT_ID")
    if iter<0:
        refinement_id = refinement_list.index.max()+1+iter
    else:
        refinement_id = iter
    refinement_package_asset_id = refinement_list.loc[refinement_id, "REFINEMENT_PACKAGE_ASSET_ID"]

    refinement_package_asset_table = pd.read_sql_table("REFINEMENT_PACKAGE_ASSETS", db).set_index("REFINEMENT_PACKAGE_ASSET_ID")
    stack_filename = str(refinement_package_asset_table.loc[refinement_package_asset_id, "STACK_FILENAME"])

    refinement_input_table_name = "REFINEMENT_PACKAGE_CONTAINED_PARTICLES_%s" % (refinement_package_asset_id)
    input_table = pd.read_sql_table(refinement_input_table_name, db).set_index("POSITION_IN_STACK")

    if "PARENT_IMAGE_ASSET_ID" in input_table:
        image_asset_table = pd.read_sql_table("IMAGE_ASSETS", db).set_index("IMAGE_ASSET_ID")

        input_table_group = input_table.groupby("PARENT_IMAGE_ASSET_ID")
        for image_asset_id, mgraphParticles in input_table_group:
            if int(image_asset_id)<1: continue  # sometimes cisTEM db file set this field to -1. why?
            input_table.loc[mgraphParticles.index, "FILENAME"] = image_asset_table.loc[image_asset_id, "FILENAME"]

    refinement_result_table_name = "REFINEMENT_RESULT_%s_%s" % (refinement_id, 1)
    result_cistem = pd.read_sql_table(refinement_result_table_name, db).set_index("POSITION_IN_STACK")
    ...
    data["rlnImageName"] = data_cistem["POSITION_IN_STACK"].astype(int).map('{:06d}'.format) + "@" + stack_filename

    mapping = {"PSI": "rlnAnglePsi", "THETA": "rlnAngleTilt", "PHI": "rlnAngleRot", "XSHIFT": "rlnOriginX", "YSHIFT": "rlnOriginY", "DEFOCUS1": "rlnDefocusU", "DEFOCUS2": "rlnDefocusV", "DEFOCUS_ANGLE": "rlnDefocusAngle", "PHASE_SHIFT": "rlnPhaseShift", "LOGP": "rlnLogLikeliContribution"}
    mapping.update({"FILENAME":"rlnMicrographName", "X_POSITION":"rlnCoordinateX", "Y_POSITION":"rlnCoordinateY", "PIXEL_SIZE":"rlnMagnification", "SPHERICAL_ABERRATION":"rlnSphericalAberration", "MICROSCOPE_VOLTAGE":"rlnVoltage", "AMPLITUDE_CONTRAST":"rlnAmplitudeContrast"})
        ...
        if key in mapping:
            if key == "PIXEL_SIZE":
                ...
                if key in "X_POSITION Y_POSITION XSHIFT YSHIFT":
                    data_cistem[key] = ((data_cistem[key].astype(float)/data_cistem["PIXEL_SIZE"].astype(float))).round(3)
...
def dataframe_normalize_filename(data, alternative_folders=[], ignore_bad_particle_path=0, ignore_bad_micrograph_path=1):
    if ignore_bad_particle_path>=2: return data

    def getRealFileName(filename, source_path, alternative_folders=[], ignore_bad_path=0):
        if filename in getRealFileName.mapping:
            return getRealFileName.mapping[filename]

        basenames = [] 
        if not os.path.isabs(filename): basenames.append( filename )
        ...
        if relion_folder is not None: folders.append( relion_folder )
        ...
        if match:
            ...
            allfiles = glob.glob( os.path.join( os.path.dirname(match), "*" + os.path.splitext(match)[-1] ) )
            for f in allfiles:
                tmp_basename = os.path.splitext(os.path.basename(f))[0] + suffix
                getRealFileName.mapping[os.path.join(filename_dir, tmp_basename)] = os.path.normpath(f)
        else:
            if ignore_bad_path:
                ret = filename
            elif match_link:
                msg = "ERROR: image %s in file %s is found at %s that is a broken link to %s" % (filename, source_path, os.path.normpath(match_link), os.readlink(match_link))
                color_print(msg)
                sys.exit(-1)
            else:
    ...
    cache = {}
    def buildFileNameCache(filenames, source_path, alternative_folders=[], ignore_bad_path=0):
        for fi, filename in enumerate(filenames):
            cache[filename] = getRealFileName(filename, source_path, alternative_folders, ignore_bad_path)

    attrs = []
    attrs_with_at = []
    for attr in "rlnImageName rlnMicrographName rlnMicrographMovieName".split():
        if attr in data:
            if attr == "rlnImageName": ignore_bad_path = ignore_bad_particle_path
            else: ignore_bad_path = ignore_bad_micrograph_path
            if "@" in data[attr][0]:
...
def relion_astigmatism_to_eman(rlnDefocusU, rlnDefocusV, rlnDefocusAngle):
    ...
    dfdiff = math.fabs(rlnDefocusU - rlnDefocusV) / 2 / 1e4  # rlnDefocusU can be larger or smaller than rlnDefocusV
    if rlnDefocusU > rlnDefocusV:
        dfang = math.fmod(rlnDefocusAngle + 360. + 90., 360.)  # largest defocus direction to smallest defocus direction
    else:
...
def get_dataframe_convention(data):
    try:
        ...
        if any(k in data for k in "rlnImageName rlnMicrographName rlnMicrographMovieName rlnVoltage".split()):
            c = "relion"
        elif any(k in data for k in "blob/path micrograph_blob/path movie_blob/path location/micrograph_path".split()):
            c = "cryosparc"
        else:
