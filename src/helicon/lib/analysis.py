...
def down_scale(data, target_apix, apix_orig):
  if target_apix == apix_orig:
    ...
    if target_apix < apix_orig:
      helicon.color_print(f"WARNING: the input image pixel size ({apix_orig}) is larger than --target_apix2d={target_apix}. Down-scaling skipped")
  return data
...
class AgglomerativeClusteringWithMinSize(AgglomerativeClustering):
    def __init__(self, min_cluster_size=2, n_clusters=2, metric='euclidean', memory=None, connectivity=None, compute_full_tree='auto', linkage='ward', distance_threshold=None):
        super().__init__(n_clusters=n_clusters, metric=metric, memory=memory, connectivity=connectivity, compute_full_tree=compute_full_tree, linkage=linkage, distance_threshold=distance_threshold)
        ...
        while True:
            ...
            for small_cluster in small_clusters:
                ...
                for point in small_cluster_points:
                    ...
                    nearest_point = valid_points[np.argmin(distances[point, valid_points])]
